...
| Join by removing one LF separator | Leading line keeps its ID, including an empty leading line. A trailing landmark transfers to it only if it has no landmark; otherwise the trailing landmark detaches. |
...
Detached landmarks retain their original line UUID and explicit detached state in metadata. They do not resolve to a nearby line. Undo or explicit move-to-current-line repair can resolve them. The direct LF-join transfer above is the sole automatic transfer rule. `snapshot.isDetached(landmark)` exposes the state. Landmark UUIDs use a distinct `LandmarkID` type; line and document UUIDs retain Crawl's existing representation.
