...
public struct HistorySettings: Codable, Equatable, Sendable {
    ...
    private enum CodingKeys: String, CodingKey { case byteBudget, minimumRecentCount, idleInterval, policyVersion }
    ...
    public init(from decoder: Decoder) throws {
        ...
        if try values.decodeIfPresent(Int.self, forKey: .policyVersion) == nil, idleInterval == 30 { idleInterval = 60 }
    }

    public init(byteBudget: Int = 256 * 1024 * 1024, minimumRecentCount: Int = 20,
    ...
    func validate() throws {
        guard byteBudget > 0, minimumRecentCount > 0, minimumRecentCount <= 100_000,
              idleInterval.isFinite, idleInterval >= 1, idleInterval <= 86_400 else {
...
enum HistoryRevisionFormat {
    ...
    static func encode(_ snapshot: DocumentSnapshot, reason: String, timestamp: Date,
                       milestone: Bool = false, id: UUID = UUID()) throws -> (Data, HistoryRevisionMetadata) {
        let payload = try PersistenceFormat.encode(snapshot)
        let metadata = HistoryRevisionMetadata(id: id, documentID: snapshot.documentID,
        ...
        let content = Content(metadata: metadata, compression: useCompression ? "lzfse" : "none",
            uncompressedBytes: payload.count, payloadHash: PersistenceFormat.checksum(payload),
            payload: useCompression ? compressed : payload)
        let envelope = Envelope(version: version, content: content,
    ...
    static func decode(_ data: Data) throws -> HistoryRevision {
        ...
        do {
            ...
            guard envelope.checksum == PersistenceFormat.checksum(try encodeJSON(content)),
                  ...
                  content.uncompressedBytes <= PersistenceFormat.maximumBytes else { throw StoreError.invalidPayload }
            let payload: Data
    ...
    static func validate(_ metadata: HistoryRevisionMetadata) throws {
        guard metadata.generation >= 0, metadata.timestamp.timeIntervalSinceReferenceDate.isFinite,
              ...
              metadata.stateHash.utf8.allSatisfy({ (48...57).contains($0) || (97...102).contains($0) }) else {
            throw StoreError.invalidPayload
        }
    }
}
