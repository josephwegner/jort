...
struct RecoveryCheckpoints {
    ...
    private func manifest() throws -> Manifest? {
        ...
        guard manifest.version == 1, !manifest.entries.isEmpty, manifest.entries.count <= 2,
              ...
              Set(manifest.entries.map(\.documentID)).count == 1 else { throw StoreError.invalidPayload }
        return manifest
    }
    func recover() throws -> DocumentSnapshot {
        guard let manifest = try manifest() else {
            return try PersistenceFormat.decode(Data(contentsOf: directory.appendingPathComponent("Recovery.json"))).snapshot
        }
    ...
    private func verified(_ entry: Entry) throws -> DocumentSnapshot? {
        do {
            ...
            guard snapshot.revision == entry.revision, snapshot.documentID == entry.documentID else { return nil }
            return snapshot
    ...
    func publish(_ data: Data, snapshot: DocumentSnapshot) throws {
        ...
        var retained: [Entry] = []
        for entry in previous.sorted(by: { $0.revision > $1.revision }) where entry.documentID == snapshot.documentID {
            if try verified(entry) != nil { retained = [entry]; break }
        }
        ...
        guard stored == data, try PersistenceFormat.decode(stored).snapshot == snapshot else { throw StoreError.invalidPayload }
        let entry = Entry(slot: slot, revision: snapshot.revision, documentID: snapshot.documentID, checksum: PersistenceFormat.checksum(data))
    ...
    private func publishFile(_ data: Data, to destination: URL) throws {
        ...
        guard Darwin.rename(temporary.path, destination.path) == 0 else { throw StoreError.io("checkpoint rename: \(errno)") }
