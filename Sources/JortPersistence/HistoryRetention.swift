...
extension Connection {
  func pruneHistory(inject: (StoreStage) throws -> Void) throws -> HistoryPruneResult {
    ...
    do {
      ...
      for entry in entries.dropFirst(settings.minimumRecentCount).reversed()
      where bytes > Int64(settings.byteBudget) {
      ...
      return HistoryPruneResult(
        removedCount: removed, retainedBytes: bytes,
        exceedsBudget: bytes > Int64(settings.byteBudget))
    } catch {
      let primary = error
      do { try execute("ROLLBACK") } catch {
        NSLog("Jort pruning rollback secondary error: %@", String(describing: error))
      }
      throw primary
    }
  }
