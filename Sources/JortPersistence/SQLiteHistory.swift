  ...
  public func retain(
      ...
      verified.metadata.stateHash == metadata.stateHash
    {
      if milestone && !verified.metadata.milestone {
        let (protectedPayload, protectedMetadata) = try HistoryRevisionFormat.encode(
          verified.snapshot,
          reason: verified.metadata.reason, timestamp: verified.metadata.timestamp,
          milestone: true, id: verified.metadata.id)
        return try connection.insertHistory(
          payload: protectedPayload, metadata: protectedMetadata,
          replacing: latest.sequence
        ) { try self.historyInjection($0) }
      }
      return latest
    }
...
extension Connection {
  ...
  func validateHistorySchema() throws {
    for (table, expected) in [
      (
        "history_revisions",
        ["sequence:INTEGER:0:1", "metadata:BLOB:1:0", "metadata_hash:TEXT:1:0", "payload:BLOB:1:0"]
      ),
    ...
    let definitions = try prepare(
      "SELECT name,sql FROM sqlite_master WHERE type='table' AND name IN ('history_revisions','history_settings')"
    )
    defer { sqlite3_finalize(definitions) }
    var step = sqlite3_step(definitions)
    while step == SQLITE_ROW {
      guard let name = sqlite3_column_text(definitions, 0),
        let definition = sqlite3_column_text(definitions, 1)
      else {
        throw StoreError.malformedSchema
      }
      let sql = String(cString: definition).lowercased().filter { !$0.isWhitespace }
      let required =
        String(cString: name) == "history_revisions"
  ...
  func historyEntries(before sequence: Int64?, limit: Int) throws -> [HistoryEntry] {
    guard (1...500).contains(limit), sequence == nil || sequence! > 0 else {
      throw StoreError.invalidPayload
    }
    let sql =
      "SELECT sequence,metadata,metadata_hash,length(payload) FROM history_revisions"
    ...
    while step == SQLITE_ROW {
      ...
      entries.append(
        HistoryEntry(
          sequence: sqlite3_column_int64(statement, 0), metadata: metadata,
          storedBytes: Int(sqlite3_column_int64(statement, 3))
  ...
  func historyRevision(sequence: Int64) throws -> HistoryRevision {
    ...
    let statement = try prepare(
      "SELECT metadata,metadata_hash,payload FROM history_revisions WHERE sequence=?")
    defer { sqlite3_finalize(statement) }
  ...
  func insertHistory(
    payload: Data, metadata: HistoryRevisionMetadata,
    replacing existingSequence: Int64? = nil,
    preservingSequence: Int64? = nil,
    inject: (StoreStage) throws -> Void
  ) throws -> HistoryEntry {
    ...
    do {
      ...
      do {
        ...
        if let preservingSequence {
          try check(sqlite3_bind_int64(statement, 4, preservingSequence))
        }
      ...
      guard try historyRevision(sequence: sequence).metadata == metadata else {
        throw StoreError.invalidPayload
      }
      try inject(.historyCommit)
      try execute("COMMIT")
      return HistoryEntry(
        sequence: sequence, metadata: metadata, storedBytes: payload.count + encodedMetadata.count)
    } catch {
      let primary = error
      do { try execute("ROLLBACK") } catch {
        NSLog("Jort history rollback secondary error: %@", String(describing: error))
      }
      throw primary
    }
  }
  ...
  func writeHistorySettings(_ settings: HistorySettings) throws {
    try settings.validate()
    let statement = try prepare(
      "INSERT INTO history_settings(id,payload) VALUES(1,?) ON CONFLICT(id) DO UPDATE SET payload=excluded.payload"
    )
    defer { sqlite3_finalize(statement) }
  ...
  private func historyMetadata(_ statement: OpaquePointer, column: Int32, hashColumn: Int32) throws
    -> HistoryRevisionMetadata
  {
    ...
    guard let hash = sqlite3_column_text(statement, hashColumn),
      String(cString: hash) == PersistenceFormat.checksum(data)
  ...
  private func bindHistoryBlob(_ data: Data, to statement: OpaquePointer, column: Int32) throws {
    ...
    try data.withUnsafeBytes { bytes in
      try check(
        sqlite3_bind_blob(
          statement, column, bytes.baseAddress, Int32(data.count),
