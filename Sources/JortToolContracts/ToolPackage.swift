...
public struct ToolManifest: Codable, Equatable, Sendable {
  ...
  public init(
    id: String, version: Int = 1, name: String, command: String, description: String = "",
  ...
  public func validate() throws {
    guard (1...2).contains(schemaVersion), entryContract == 1,
      ...
      schemaVersion == 1 || executor != nil
    else { throw ToolPackageError.unsupportedContract }
    guard version > 0, id.utf8.count <= 256, (basedOnTemplateID?.utf8.count ?? 0) <= 256,
      ...
      outputOperation != .replaceContext || inputMode == .contextual
    else {
...
public enum ToolPackageError: Error, Equatable, Sendable {
  case unsupportedContract, invalidManifest, invalidSource, sizeLimit, invalidPath, conflict,
    unavailable
}
...
public struct ToolPackage: Equatable, Sendable {
  ...
  public var source: String {
    get {
      if case .javascript(let source) = implementation { return source }
      return ""
    }
    set { implementation = .javascript(newValue) }
  }
  public var instructions: String {
    if case .model(let instructions) = implementation { return instructions }
    return ""
  }
  public init(manifest: ToolManifest, source: String) {
    self.manifest = manifest
    implementation = .javascript(source)
  }
  public init(manifest: ToolManifest, instructions: String) {
    self.manifest = manifest
    implementation = .model(instructions: instructions)
  }
  public func validate() throws {
    try manifest.validate()
    if manifest.executorType == .model {
      guard case .model = implementation,
        !instructions.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
        instructions.utf8.count <= 32_768, !instructions.contains("\0"),
        let model = manifest.modelID, !model.isEmpty, model.utf8.count <= 256
      else { throw ToolPackageError.invalidManifest }
      return
    }
    guard case .javascript = implementation, manifest.modelID == nil else {
      throw ToolPackageError.invalidManifest
    }
    guard !source.isEmpty, source.utf8.count <= 262_144,
      !source.contains("\0")
    else { throw ToolPackageError.invalidSource }
  }
