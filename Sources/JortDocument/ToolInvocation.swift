...
public struct ToolAnchor: Codable, Equatable, Sendable {
    ...
    public init(lineID: UUID, offset: Int) { self.lineID = lineID; self.offset = offset }
    public func resolve(in lines: [LineMeta]) -> Int? {
        guard let line = lines.first(where: { $0.id == lineID }), offset >= 0, offset <= line.length else { return nil }
        return line.location + offset
    }
    public static func at(_ offset: Int, in lines: [LineMeta]) -> Self? {
        guard offset >= 0, let line = lines.last(where: { $0.location <= offset }), offset <= line.location + line.length else { return nil }
...
public struct ToolAnchoredRange: Codable, Equatable, Sendable {
    ...
    public init(_ range: NSRange, lines: [LineMeta]) throws {
        guard range.location >= 0, range.length >= 0, range.length <= Int.max - range.location,
              ...
              let end = ToolAnchor.at(NSMaxRange(range), in: lines) else { throw DocumentError.invalidRange }
        self.start = start; self.end = end
    }
    public func resolve(in lines: [LineMeta]) -> NSRange? {
        guard let a = start.resolve(in: lines), let b = end.resolve(in: lines), b >= a else { return nil }
...
public struct ToolInvocation: Codable, Equatable, Identifiable, Sendable {
    ...
    public init(packageID: String, packageVersion: Int, entryContract: Int, inputMode: String,
        ...
        self.packageID = packageID; self.packageVersion = packageVersion; self.entryContract = entryContract
        self.inputMode = inputMode; self.outputOperation = outputOperation; self.command = command
        self.token = token; self.scope = scope; self.sourceHash = sourceHash
    }
    ...
    public func validated(in snapshot: DocumentSnapshot) -> Bool {
        ...
        if let output {
            guard let range = output.resolve(in: snapshot.lines), NSMaxRange(range) <= text.length,
                  ...
                  Self.hash(text.substring(with: range)) == outputHash else { return false }
            let intersection = NSIntersectionRange(scope, range)
            if intersection.length > 0 {
                source = (source as NSString).replacingCharacters(in: NSRange(location: intersection.location - scope.location, length: intersection.length), with: "")
            }
        }
    ...
    public static func sanitized(_ values: [Self], in snapshot: DocumentSnapshot) -> [Self] {
        ...
        let intervals = valid.compactMap { invocation -> (UUID, Int, Int)? in
            ...
            return (invocation.id, scope.location, max(NSMaxRange(scope), invocation.output?.resolve(in: snapshot.lines).map(NSMaxRange) ?? NSMaxRange(scope)))
        }.sorted { $0.1 < $1.1 }
        var cluster: [UUID] = [], end = -1
        for (id, start, upper) in intervals {
            if !seen.insert(id).inserted { rejected.insert(id) }
            if start < end {
                rejected.formUnion(cluster); rejected.insert(id)
                cluster.append(id); end = max(end, upper)
            } else {
                cluster = [id]; end = upper
            }
        }
...
public enum ToolRangeEditing {
    public static func intersectsLock(_ edit: NSRange, snapshot: DocumentSnapshot) -> Bool {
        snapshot.invocations.contains { invocation in
            ...
            return edit.length == 0 ? edit.location > range.location && edit.location < NSMaxRange(range) : NSIntersectionRange(edit, range).length > 0
        }
    }

    public static func remap(_ invocations: [ToolInvocation], from old: DocumentSnapshot, to new: DocumentSnapshot,
                             edit: NSRange, replacementLength: Int) -> [ToolInvocation] {
        ...
        func range(_ value: ToolAnchoredRange, grow: Bool, growStart: Bool = false) -> ToolAnchoredRange? {
            guard let source = value.resolve(in: old.lines) else { return nil }
            let a = offset(source.location, end: !growStart && edit.length == 0 && source.location == edit.location)
        ...
        return invocations.compactMap { original in
            guard let oldToken = original.token.resolve(in: old.lines),
                  NSIntersectionRange(oldToken, edit).length == 0 else { return nil }
            var value = original
            guard let token = range(value.token, grow: false),
                  let scope = range(value.scope, grow: !value.isLocked && !value.inputMode.hasPrefix("ephemeral"),
