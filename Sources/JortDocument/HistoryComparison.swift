...
public struct HistoryToolAnnotation: Equatable, Sendable {
  ...
  public var label: String {
    command + " · "
      + (phase == .pending
        ? "Pending merge" : phase == .inputting ? "Input" : phase.rawValue.capitalized)
  }
}
...
public struct HistoryComparison: Sendable {
  ...
  public static func compare(_ old: DocumentSnapshot, _ new: DocumentSnapshot) throws
    -> HistoryComparison
  {
    ...
    try old.validate()
    try new.validate()
    let oldIndices = Dictionary(
      uniqueKeysWithValues: old.lines.enumerated().map { ($0.element.id, $0.offset) })
    ...
    let shared = new.lines.enumerated().compactMap { index, line in
      oldIndices[line.id].map { (old: $0, new: index) }
    }
    ...
    while index >= 0 {
      anchors.append(shared[index])
      index = previous[index]
    }
    anchors.reverse()
    anchors.append((old.lines.count, new.lines.count))
    let oldText = old.text as NSString, newText = new.text as NSString
    let oldEmoji = Dictionary(
      uniqueKeysWithValues: old.landmarks.filter { !$0.detached }.map { ($0.lineID, $0.emoji) })
    let newEmoji = Dictionary(
      uniqueKeysWithValues: new.landmarks.filter { !$0.detached }.map { ($0.lineID, $0.emoji) })
    func toolAnnotations(_ snapshot: DocumentSnapshot) -> [UUID: [HistoryToolAnnotation]] {
      var result: [UUID: [HistoryToolAnnotation]] = [:]
      let indices = Dictionary(
        uniqueKeysWithValues: snapshot.lines.enumerated().map { ($0.element.id, $0.offset) })
      for invocation in snapshot.invocations {
        guard let scope = invocation.scope.resolve(in: snapshot.lines),
          let first = indices[invocation.scope.start.lineID]
        else { continue }
        ...
        for line in snapshot.lines[first...] {
          ...
          let containsOutput =
            output.map {
              NSIntersectionRange($0, NSRange(location: line.location, length: line.length)).length
                > 0
                || $0.length == 0 && $0.location >= line.location
                  && $0.location < line.location + max(1, line.length)
            } ?? false
          result[line.id, default: []].append(
            .init(
              command: invocation.command, phase: invocation.phase, containsOutput: containsOutput))
        }
      }
      return result
    }
    ...
    func remove(_ i: Int) {
      rows.append(
        .init(
          kind: .removed, oldOrdinal: i + 1, newOrdinal: nil,
          text: content(oldText, old.lines[i]), emoji: oldEmoji[old.lines[i].id],
          tools: oldTools[old.lines[i].id] ?? []))
      removed += 1
    }
    func add(_ i: Int) {
      rows.append(
        .init(
          kind: .added, oldOrdinal: nil, newOrdinal: i + 1,
          text: content(newText, new.lines[i]), emoji: newEmoji[new.lines[i].id],
          tools: newTools[new.lines[i].id] ?? []))
      added += 1
    }
    for anchor in anchors {
      ...
      if content(oldText, old.lines[a]) == text
        && oldEmoji[old.lines[a].id] == newEmoji[new.lines[b].id]
        && oldTools[old.lines[a].id] == newTools[new.lines[b].id]
      {
        rows.append(
          .init(
            kind: .unchanged, oldOrdinal: a + 1, newOrdinal: b + 1, text: text,
            emoji: newEmoji[new.lines[b].id], tools: newTools[new.lines[b].id] ?? []))
      } else {
        remove(a)
        add(b)
      }
    ...
    return .init(
      lines: rows, added: added, removed: removed,
      metadataChanged: old.lines != new.lines || old.landmarks != new.landmarks
