    ...
    public static func compare(_ old: DocumentSnapshot, _ new: DocumentSnapshot) throws -> HistoryComparison {
        ...
        func remove(_ i: Int) {
            rows.append(.init(kind: .removed, oldOrdinal: i + 1, newOrdinal: nil,
                text: content(oldText, old.lines[i]), emoji: oldEmoji[old.lines[i].id])); removed += 1
        }
        func add(_ i: Int) {
            rows.append(.init(kind: .added, oldOrdinal: nil, newOrdinal: i + 1,
                text: content(newText, new.lines[i]), emoji: newEmoji[new.lines[i].id])); added += 1
        }
        for anchor in anchors {
            ...
            if content(oldText, old.lines[a]) == text && oldEmoji[old.lines[a].id] == newEmoji[new.lines[b].id] {
                rows.append(.init(kind: .unchanged, oldOrdinal: a + 1, newOrdinal: b + 1, text: text, emoji: newEmoji[new.lines[b].id]))
            } else { remove(a); add(b) }
            a += 1; b += 1
        }
