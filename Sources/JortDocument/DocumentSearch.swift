...
public struct SearchMatch: Equatable, Sendable {
    ...
    public func resolve(in snapshot: DocumentSnapshot) -> NSRange? {
        guard snapshot.documentID == documentID, snapshot.revision >= generation,
              ...
              lineRange.location >= 0, lineRange.location <= line.length else { return nil }
...
public enum DocumentSearch {
    public static func scan(_ snapshot: DocumentSnapshot, query: String, options: SearchOptions = .init(), limit: Int = 1_000) throws -> SearchPage {
        ...
        let regex = try NSRegularExpression(pattern: pattern, options: options.caseSensitive ? [] : [.caseInsensitive])
        let source = snapshot.text as NSString
        var matches: [SearchMatch] = [], lineIndex = 0, hasMore = false
        regex.enumerateMatches(in: snapshot.text, options: [.reportProgress], range: NSRange(location: 0, length: source.length)) { match, _, stop in
            if Task.isCancelled { stop.pointee = true; return }
            guard let range = match?.range else { return }
            if matches.count == limit { hasMore = true; stop.pointee = true; return }
            while lineIndex + 1 < snapshot.lines.count, snapshot.lines[lineIndex + 1].location <= range.location { lineIndex += 1 }
            guard snapshot.lines.indices.contains(lineIndex) else { stop.pointee = true; return }
            ...
            let context = source.rangeOfComposedCharacterSequences(for: NSRange(location: start, length: length))
            var snippet = "", snippetLength = 0
            for character in source.substring(with: context) {
                if character.isNewline { break }
                let piece = String(character)
                guard snippetLength + piece.utf16.count <= 180 else { snippet += "…"; break }
                snippet += piece; snippetLength += piece.utf16.count
            }
            matches.append(SearchMatch(documentID: snapshot.documentID, generation: snapshot.revision, lineID: line.id,
                lineRange: NSRange(location: range.location - line.location, length: range.length),
                matchedTextHash: hash(source.substring(with: range)), ordinal: lineIndex + 1, snippet: snippet))
        }
        try Task.checkCancellation()
