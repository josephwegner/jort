...
public struct LineMeta: Codable, Equatable, Sendable {
  ...
  public init(
    id: UUID = UUID(), location: Int, length: Int, createdAt: Date? = nil, lastEditedAt: Date? = nil
  ) {
    self.id = id
...
struct DocumentState: Equatable {
  ...
  static func ranges(in text: String) -> [NSRange] {
    ...
    if source.length == 0 || source.character(at: source.length - 1) == 10
      || source.character(at: source.length - 1) == 13
    {
  ...
  mutating func replaceText(
      ...
      oldSource.length - range.length + length == source.length
    {
      ...
      { tail += 1 }
      prefix = head
      oldEnd = old.count - tail
      newEnd = new.count - tail
    }
    let delta = source.length - oldSource.length
    let directJoin =
      oldEnd == prefix + 1 && newEnd == prefix && oldSource.character(at: prefix) == 10
    ...
    let window = source.substring(
      with: NSRange(location: startOffset, length: newWindowEnd - startOffset))
    ...
    let updated = ranges.map { localRange -> LineMeta in
      ...
      if range.location < lines[leading].location {
        ...
        let endsWithSeparator =
          leadingEnd > 0 && [10, 13].contains(Int(oldSource.character(at: leadingEnd - 1)))
        let deletedWholeLeading =
          !directJoin && endsWithSeparator && prefix == lines[leading].location
        ...
        } else if !deletedWholeLeading
          || oldEnd <= lines[leading].location + lines[leading].length - 1
        {
      ...
      if content.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
        ...
        let unchanged =
          inherited.map {
            oldSource.substring(with: NSRange(location: $0.location, length: $0.length)) == content
          } ?? false
  ...
  func validate() throws {
    ...
    for landmark in landmarks {
      ...
      if !landmark.detached {
        guard lineIDs.contains(landmark.lineID), attached.insert(landmark.lineID).inserted else {
          throw DocumentError.invalidState
        }
      }
    }
    ...
    for line in lines {
      guard line.location >= 0, line.length >= 0, line.location <= source.length,
        line.length <= source.length - line.location
      else { throw DocumentError.invalidState }
      let empty = source.substring(with: NSRange(location: line.location, length: line.length))
        .trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      guard
        empty
          ? (line.createdAt == nil && line.lastEditedAt == nil)
          : (line.createdAt != nil && line.lastEditedAt != nil),
        line.createdAt == nil || line.createdAt! <= line.lastEditedAt!
      else { throw DocumentError.invalidState }
    }
    let ranges = Self.ranges(in: text)
    guard ranges.count == lines.count,
      Set(lines.map(\.id)).count == lines.count,
      zip(ranges, lines).allSatisfy({ $0.location == $1.location && $0.length == $1.length })
    else {
      throw DocumentError.invalidState
    }
  }
