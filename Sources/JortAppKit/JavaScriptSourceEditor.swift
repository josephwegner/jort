...
@MainActor public final class SourceTextView: NSTextView {
  ...
  public override func shouldChangeText(in affectedCharRange: NSRange, replacementString: String?)
    -> Bool
  {
    let replacement = replacementString ?? ""
    let proposed = (string as NSString).replacingCharacters(
      in: affectedCharRange, with: replacement)
    guard proposed.utf8.count <= maximumUTF8Bytes else {
      rejectedOversize?()
      NSSound.beep()
      return false
    }
...
@MainActor final class SourceLineRuler: NSRulerView {
  ...
  override func drawHashMarksAndLabels(in rect: NSRect) {
    guard let textView = sourceView, let layout = textView.layoutManager,
    ...
    while cursor < text.length
      && cursor < layout.characterRange(forGlyphRange: glyphRange, actualGlyphRange: nil).location
    {
    ...
    while glyph < NSMaxRange(glyphRange) {
      ...
      label.draw(
        at: NSPoint(
          x: ruleThickness - size.width - 8, y: fragment.minY + textView.textContainerInset.height),
        withAttributes: attributes)
      glyph = max(NSMaxRange(lineGlyph), glyph + 1)
      line += 1
    }
  }
}
...
@MainActor public final class JavaScriptSourceEditor: NSView, NSTextViewDelegate {
  ...
  public var source: String {
    ...
    set {
      ...
      textView.sourceUndoManager.removeAllActions()
      updating = false
  ...
  public var isSourceEditable: Bool {
    ...
    set {
      ...
      textView.backgroundColor = newValue ? .textBackgroundColor : .controlBackgroundColor
    }
  }
  ...
  public override init(frame frameRect: NSRect) {
    ...
    textView.setAccessibilityLabel("JavaScript source")
    ...
    status.font = .monospacedDigitSystemFont(ofSize: 10, weight: .regular)
    status.setAccessibilityLabel("Source position")
    NSLayoutConstraint.activate([
      scrollView.leadingAnchor.constraint(equalTo: leadingAnchor),
      scrollView.trailingAnchor.constraint(equalTo: trailingAnchor),
      scrollView.topAnchor.constraint(equalTo: topAnchor),
      scrollView.bottomAnchor.constraint(equalTo: status.topAnchor, constant: -4),
  ...
  public func reveal(_ range: ToolSourceRange) {
    let safe = NSRange(
      location: min(range.location, textView.string.utf16.count),
      length: min(range.length, max(0, textView.string.utf16.count - range.location)))
