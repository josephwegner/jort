...
@MainActor
final class HistoryWorkspaceController: NSViewController, NSTableViewDataSource, NSTableViewDelegate
{
  ...
  let mode = NSSegmentedControl(
    labels: ["Changes", "Snapshot"], trackingMode: .selectOne, target: nil, action: nil)
  ...
  init(store: any HistoryStore) {
    model = HistoryBrowserModel(store: store)
  ...
  override func loadView() {
    ...
    view.layer?.backgroundColor = EditorMetrics.canvas.cgColor
    let rail = NSView(), footer = NSView(), actions = NSView(), divider = NSBox()
    divider.boxType = .separator
    for surface in [rail, footer] {
      surface.wantsLayer = true
      surface.layer?.backgroundColor = EditorMetrics.chrome.cgColor
    }
    let title = NSTextField(labelWithString: "Version History")
    title.font = .systemFont(ofSize: 15, weight: .semibold)
    revisionScroll.hasVerticalScroller = true
    revisionScroll.documentView = revisions
    revisionScroll.contentView.postsBoundsChangedNotifications = true
    NotificationCenter.default.addObserver(
      self, selector: #selector(updateVisibleRevisions), name: NSView.boundsDidChangeNotification,
      object: revisionScroll.contentView)
    revisions.addTableColumn(NSTableColumn(identifier: .init("revision")))
    revisions.headerView = nil
    revisions.rowHeight = 76
    revisions.style = .plain
    revisions.backgroundColor = EditorMetrics.chrome
    revisions.delegate = self
    revisions.dataSource = self
    ...
    snapshotScroll.documentView = snapshot
    snapshotScroll.hasVerticalScroller = true
    ...
    snapshotScroll.hasVerticalRuler = true
    snapshotScroll.rulersVisible = true
    let changeColumn = NSTableColumn(identifier: .init("change"))
    changeColumn.width = 800
    changes.addTableColumn(changeColumn)
    changes.headerView = nil
    changes.rowHeight = 24
    changes.style = .plain
    changes.intercellSpacing = .zero
    changes.selectionHighlightStyle = .none
    changes.backgroundColor = EditorMetrics.canvas
    changes.delegate = self
    changes.dataSource = self
    ...
    changes.target = self
    changes.doubleAction = #selector(expandSelection)
    changesScroll.documentView = changes
    ...
    let railWidth = rail.widthAnchor.constraint(equalToConstant: 280)
    ...
    NSLayoutConstraint.activate([
      ...
      rail.bottomAnchor.constraint(equalTo: footer.topAnchor),
      footer.leadingAnchor.constraint(equalTo: view.leadingAnchor),
      footer.trailingAnchor.constraint(equalTo: view.trailingAnchor),
      footer.bottomAnchor.constraint(equalTo: view.bottomAnchor),
      footer.heightAnchor.constraint(equalToConstant: EditorMetrics.footerHeight),
      title.topAnchor.constraint(equalTo: rail.topAnchor, constant: 16),
      title.leadingAnchor.constraint(equalTo: rail.leadingAnchor, constant: 16),
      mode.topAnchor.constraint(equalTo: title.bottomAnchor, constant: 12),
      mode.leadingAnchor.constraint(equalTo: title.leadingAnchor),
      mode.trailingAnchor.constraint(equalTo: rail.trailingAnchor, constant: -16),
      revisionScroll.topAnchor.constraint(equalTo: mode.bottomAnchor, constant: 12),
      revisionScroll.leadingAnchor.constraint(equalTo: rail.leadingAnchor, constant: 6),
      revisionScroll.trailingAnchor.constraint(equalTo: rail.trailingAnchor, constant: -6),
      revisionScroll.bottomAnchor.constraint(equalTo: actions.topAnchor),
      actions.leadingAnchor.constraint(equalTo: rail.leadingAnchor),
      actions.trailingAnchor.constraint(equalTo: rail.trailingAnchor),
      actions.bottomAnchor.constraint(equalTo: rail.bottomAnchor),
      actions.heightAnchor.constraint(equalToConstant: 64),
      divider.leadingAnchor.constraint(equalTo: actions.leadingAnchor, constant: 12),
      divider.trailingAnchor.constraint(equalTo: actions.trailingAnchor, constant: -12),
      divider.topAnchor.constraint(equalTo: actions.topAnchor),
      done.trailingAnchor.constraint(equalTo: actions.trailingAnchor, constant: -12),
      done.centerYAnchor.constraint(equalTo: actions.centerYAnchor),
      restore.leadingAnchor.constraint(equalTo: actions.leadingAnchor, constant: 12),
      restore.centerYAnchor.constraint(equalTo: actions.centerYAnchor),
      status.leadingAnchor.constraint(equalTo: footer.leadingAnchor, constant: 12),
      status.trailingAnchor.constraint(lessThanOrEqualTo: footer.trailingAnchor, constant: -12),
      status.centerYAnchor.constraint(equalTo: footer.centerYAnchor),
    ])
    for preview in [snapshotScroll, changesScroll] {
      NSLayoutConstraint.activate([
        preview.leadingAnchor.constraint(equalTo: view.leadingAnchor),
        preview.trailingAnchor.constraint(equalTo: rail.leadingAnchor, constant: -1),
  ...
  override func viewDidAppear() {
    super.viewDidAppear()
    view.window?.makeFirstResponder(revisions)
  }
  override func viewDidLayout() {
    super.viewDidLayout()
    updateVisibleRevisions()
  }
  ...
  @objc private func updateVisibleRevisions() {
    ...
    guard range.location != NSNotFound, range.location < model.entries.count, range.length > 0
    else { return }
  ...
  @objc func selectPresentation() {
    wantsChanges = mode.selectedSegment == 0
    refresh()
  }
  ...
  private func refresh() {
    ...
    if let selectedRow {
      revisions.selectRowIndexes(IndexSet(integer: selectedRow), byExtendingSelection: false)
    }
    ...
    if let revision = model.selected {
      ...
      status.stringValue =
        "\(HistoryBrowserModel.date(revision.metadata.timestamp)) · Read-only · \(attached) landmarks"
      if let comparison = model.comparison {
        if shownComparison != revision.metadata.id {
          makeRows(comparison)
          shownComparison = revision.metadata.id
        }
      }
  ...
  private func makeRows(_ comparison: HistoryComparison) {
    ...
    while index < comparison.lines.count {
      if comparison.lines[index].kind == .unchanged {
        let start = index
        while index < comparison.lines.count && comparison.lines[index].kind == .unchanged {
          index += 1
        }
        if index - start > 8 {
          ...
          rows.append(contentsOf: (start..<index).map { .line($0) })
        }
      } else {
    ...
    let digits = String(
      max(
        comparison.lines.compactMap(\.oldOrdinal).max() ?? 1,
        comparison.lines.compactMap(\.newOrdinal).max() ?? 1)
    ).count
    changes.gutterWidth = CGFloat(max(2, digits) * 16 + 62)
    changes.tableColumns[0].width = max(
      changesScroll.contentSize.width,
      CGFloat(comparison.lines.map { $0.text.utf16.count }.max() ?? 0) * 8
        + CGFloat(max(2, digits) * 16 + 80))
    changes.reloadData()
  }
  ...
  @objc func expandSelection() {
    ...
    rows.replaceSubrange(index...index, with: range.map { .line($0) })
    changes.reloadData()
    changes.selectRowIndexes(IndexSet(integer: index), byExtendingSelection: false)
  }

  func numberOfRows(in tableView: NSTableView) -> Int {
    tableView === revisions ? model.entries.count : rows.count
  }
  func tableView(_ tableView: NSTableView, rowViewForRow row: Int) -> NSTableRowView? {
    tableView === revisions ? HistoryRevisionRow() : nil
  }
  func tableViewSelectionDidChange(_ notification: Notification) {
    if notification.object as? NSTableView === revisions { selectRow() }
  }
  ...
  func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView?
  {
    ...
    case .fold(let range):
      ...
      let button = NSButton(
        title: "Show \(range.count) unchanged lines", target: self,
        action: #selector(expandButton(_:)))
    ...
    case .line(let index):
      ...
      let digits = String(
        max(model.selected?.snapshot.lines.count ?? 1, model.baseline?.snapshot.lines.count ?? 1)
      ).count
  ...
  private func revisionCell(_ entry: HistoryEntry) -> NSView {
    ...
    let title = NSTextField(
      labelWithString: entry.metadata.map { HistoryBrowserModel.date($0.timestamp) }
    ...
    counts.font = .monospacedDigitSystemFont(ofSize: 11, weight: .medium)
    counts.textColor = .tertiaryLabelColor
    if let summary = model.summaries[entry.sequence] {
      switch summary {
      case .changes(let added, let removed):
        let value = NSMutableAttributedString(
          string: "+\(added)", attributes: [.foregroundColor: NSColor.systemGreen])
        value.append(
          NSAttributedString(
            string: "  −\(removed)", attributes: [.foregroundColor: NSColor.systemRed]))
    ...
    NSLayoutConstraint.activate([
      ...
      counts.topAnchor.constraint(equalTo: title.bottomAnchor, constant: 5),
      counts.leadingAnchor.constraint(equalTo: title.leadingAnchor),
      reason.leadingAnchor.constraint(equalTo: title.leadingAnchor),
      reason.trailingAnchor.constraint(equalTo: cell.trailingAnchor, constant: -8),
      reason.bottomAnchor.constraint(equalTo: cell.bottomAnchor, constant: -9),
    ])
    cell.setAccessibilityLabel(
      "\(title.stringValue), \(reason.stringValue), \(counts.accessibilityLabel() ?? counts.stringValue)"
    )
    return cell
  }

  @objc private func confirmRestore() {
    guard let selected = model.selected, let sequence = model.selectedSequence,
      let window = view.window, !restoring
    else { return }
    let alert = NSAlert()
    alert.messageText = "Restore this revision?"
    alert.informativeText =
      "Replace the current document with \(HistoryBrowserModel.date(selected.metadata.timestamp))? Your current text and landmarks will be retained in history, and you can undo the restore."
  ...
  func performRestore(_ sequence: Int64) async {
    ...
    do { try await onRestore(sequence) } catch {
      status.stringValue = "Restore could not complete. Your current document was not replaced."
      status.toolTip = String(describing: error)
    }
    restoring = false
    done.isEnabled = true
    mode.isEnabled = true
    restore.isEnabled = model.selected != nil
  }
}
