...
@MainActor
final class DocumentSearchController: NSViewController, NSTableViewDataSource, NSTableViewDelegate,
  NSSearchFieldDelegate
{
  ...
  override func loadView() {
    ...
    view.layer?.backgroundColor = EditorMetrics.chrome.cgColor
    view.layer?.cornerRadius = 10
    view.layer?.borderWidth = 1
    view.layer?.borderColor = EditorMetrics.separator.cgColor
    view.shadow = NSShadow()
    view.shadow?.shadowBlurRadius = 12
    view.setAccessibilityLabel("Search current document")
    query.placeholderString = "Search…"
    query.delegate = self
    query.sendsSearchStringImmediately = true
    query.setAccessibilityLabel("Document search query")
    close.target = self
    close.action = #selector(dismissSearch)
    close.isBordered = false
    close.setAccessibilityLabel("Close search")
    previous.target = self
    previous.action = #selector(previousMatch)
    next.target = self
    next.action = #selector(nextMatch)
    previous.setAccessibilityLabel("Previous search match")
    next.setAccessibilityLabel("Next search match")
    count.setAccessibilityLabel("Search result count and status")
    count.font = .systemFont(ofSize: 11)
    for button in [caseSensitive, wholeWord] {
      button.target = self
      button.action = #selector(refreshQuery)
      button.font = .systemFont(ofSize: 11)
    }
    more.target = self
    more.action = #selector(loadMore)
    let header = NSStackView(views: [query, close])
    header.spacing = 8
    let options = NSStackView(views: [caseSensitive, wholeWord])
    options.spacing = 12
    let navigation = NSStackView(views: [count, NSView(), previous, next])
    ...
    resultsArea.orientation = .vertical
    resultsArea.alignment = .leading
    resultsArea.spacing = 8
    for child in [options, navigation, scroll, more] { resultsArea.addArrangedSubview(child) }
    let stack = NSStackView(views: [header, resultsArea])
    ...
    NSLayoutConstraint.activate([
      ...
      resultsArea.widthAnchor.constraint(equalTo: stack.widthAnchor),
  ...
  func present() {
    _ = view
    view.window?.makeFirstResponder(query)
  }
  ...
  private func search() {
    ...
    model.search(
      snapshot: snapshot(), query: query.stringValue,
      options: .init(caseSensitive: caseSensitive.state == .on, wholeWord: wholeWord.state == .on),
      limit: limit)
  }
  ...
  private func reload() {
    table.reloadData()
    count.stringValue = model.message
    more.isHidden = !model.hasMore
    resultsArea.isHidden = query.stringValue.isEmpty
    updateResultsHeight()
    if !model.results.isEmpty {
      table.selectRowIndexes(IndexSet(integer: 0), byExtendingSelection: false)
    }
    previous.isEnabled = !model.results.isEmpty
    next.isEnabled = !model.results.isEmpty
  ...
  func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView?
  {
    ...
    label.setAccessibilityLabel("Line \(match.ordinal), \(match.snippet)")
    label.toolTip = match.snippet
  ...
  @objc func activateSelection() {
    ...
    if navigate?(model.results[table.selectedRow]) == true {
      count.stringValue =
        "\(table.selectedRow + 1) of \(model.results.count)\(model.hasMore ? "+" : "") matches"
    } else {
      search()
      count.stringValue = "Document changed — refreshing matches"
    }
  }
