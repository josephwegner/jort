...
@MainActor final class HistoryBrowserModel {
    ...
    func requestSummaries(_ sequences: [Int64]) {
        ...
        visibleSequences = sequences; summaryTask?.cancel()
            ...
            for sequence in sequences {
                ...
                do {
                    ...
                    if let earlier = try await store.revisions(before: sequence, limit: 1).first {
                        ...
                        try Task.checkCancellation()
                        let worker = Task.detached(priority: .utility) { try HistoryComparison.compare(old.snapshot, current.snapshot) }
                    ...
                    } else { summaries[sequence] = .initial }
                } catch {
    ...
    func loadMore() {
        ...
        listTask = Task { [weak self, store] in
            do {
                ...
                entries.append(contentsOf: page); hasMore = page.count == 100
                listTask = nil
    ...
    func select(_ sequence: Int64) {
        ...
        selectedSequence = sequence; selected = nil; baseline = nil; comparison = nil
        loading = true; message = "Verifying revision…"; onChange?()
        selectionTask = Task { [weak self, store] in
            do {
                let revision = try await store.revision(sequence: sequence)
                try Task.checkCancellation()
                guard let self, active else { return }
                selected = revision; loading = false; message = "Looking for a comparison…"; onChange?()
                ...
                if let earlier, let old = try? await store.revision(sequence: earlier.sequence) {
                    try Task.checkCancellation()
                    let work = Task.detached(priority: .userInitiated) { try HistoryComparison.compare(old.snapshot, revision.snapshot) }
                    let diff = try await withTaskCancellationHandler {
                        try await work.value
                    } onCancel: { work.cancel() }
                    try Task.checkCancellation()
                    baseline = old; comparison = diff
                    summaries[sequence] = .changes(added: diff.added, removed: diff.removed)
                    message = "Compared with \(Self.date(old.metadata.timestamp))"
                } else { message = "No earlier verified snapshot is available for comparison." }
                onChange?()
            } catch {
                guard let self, active, !Task.isCancelled else { return }
                loading = false
                ...
                } else { message = "Comparison unavailable. The complete snapshot can still be restored." }
                onChange?()
            }
        }
    }
    ...
    func cancel() {
        active = false; listTask?.cancel(); selectionTask?.cancel(); summaryTask?.cancel(); onChange = nil
    }
