    ...
    func search(snapshot: DocumentSnapshot, query: String, options: SearchOptions, limit: Int = 1_000) {
        ...
        task = Task { [weak self] in
            do {
                ...
                results = page.matches; hasMore = page.hasMore
                message = results.isEmpty ? "No matches" : "\(results.count)\(hasMore ? "+" : "") \(results.count == 1 && !hasMore ? "match" : "matches")"
                onChange?()
            } catch is CancellationError { }
            catch {
                guard let self, self.request == token else { return }
                message = "Search could not complete"; onChange?()
            }
        }
    }
    func cancel() { task?.cancel(); task = nil; request = UUID() }
}
