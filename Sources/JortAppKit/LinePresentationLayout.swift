...
@MainActor final class LinePresentationLayout: NSObject, @preconcurrency NSTextLayoutManagerDelegate
{
  ...
  func textLayoutManager(
    ...
    guard let content = textLayoutManager.textContentManager else {
      return NSTextLayoutFragment(textElement: textElement, range: nil)
    }
    return AccessoryLayoutFragment(element: textElement) {
      [weak self, weak textElement, weak content] in
      guard let self, let textElement, let content, let range = textElement.elementRange else {
        return 0
      }
  ...
  func canonicalRects(for range: NSRange) -> [NSRect] {
    ...
    manager.enumerateTextLayoutFragments(
      ...
      b = max(
        b,
        content.offset(
          from: content.documentRange.location, to: fragment.rangeInElement.endLocation))
      return fragment.layoutFragmentFrame.maxY < visibleBottom
    }
    ...
    guard clipped.length > 0 || range.length == 0 && range.location >= a && range.location <= b,
      let start = content.location(
        ...
        offsetBy: range.length == 0 ? range.location : clipped.location),
    ...
    manager.enumerateTextSegments(
      in: textRange, type: range.length == 0 ? .selection : .standard, options: [.rangeNotRequired]
    ) { _, frame, _, _ in
      ...
      if let line = lineFrames.first(where: { abs($0.midY - frame.midY) < max(2, $0.height / 2) }) {
        ...
        normalized.size.height = max(
          line.height, editor.defaultParagraphStyle?.minimumLineHeight ?? 24)
      }
      let positioned = normalized.offsetBy(
        dx: editor.textContainerOrigin.x, dy: editor.textContainerOrigin.y)
  ...
  func glyphFrame(at offset: Int) -> NSRect? {
    ...
    for run in CTLineGetGlyphRuns(shaped) as! [CTRun] {
      ...
      if let glyph = indices.firstIndex(of: localIndex) {
        glyphX = positions[glyph].x
        break
      }
    }
    ...
    return NSRect(
      x: editor.textContainerOrigin.x + fragment.layoutFragmentFrame.minX
        + line.typographicBounds.minX + line.glyphOrigin.x + glyphX,
  ...
  func accessibilityChildren() -> [Any]? {
    ...
    for band in visibleBands() {
      ...
      let frame = band.frame.offsetBy(
        dx: editor.textContainerOrigin.x, dy: editor.textContainerOrigin.y)
      if let window = editor.window {
        text.setAccessibilityFrame(window.convertToScreen(editor.convert(frame, to: nil)))
      }
