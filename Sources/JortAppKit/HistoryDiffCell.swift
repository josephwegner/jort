...
@MainActor final class HistoryDiffCell: NSTableCellView {
    ...
    init(line: HistoryChangeLine, digits: Int) {
        ...
        textField = text
        old.textColor = .secondaryLabelColor; new.textColor = .secondaryLabelColor
        let widths: [(NSTextField, CGFloat)] = [(emoji, 26), (old, numberWidth), (new, numberWidth), (sign, 20)]
    ...
    private func field(_ value: String, alignment: NSTextAlignment) -> NSTextField {
        ...
        field.font = .monospacedSystemFont(ofSize: 13, weight: .regular); field.alignment = alignment
    ...
    override func draw(_ dirtyRect: NSRect) {
        EditorMetrics.canvas.setFill(); bounds.fill()
        let tint: NSColor? = kind == .added ? .systemGreen : kind == .removed ? .systemRed : nil
        tint?.withAlphaComponent(0.12).setFill(); if tint != nil { bounds.fill() }
        let gutter = NSRect(x: 0, y: 0, width: gutterWidth, height: bounds.height)
        EditorMetrics.chrome.setFill(); gutter.fill()
        tint?.withAlphaComponent(0.07).setFill(); if tint != nil { gutter.fill() }
        EditorMetrics.separator.setFill(); NSRect(x: gutterWidth - 1, y: 0, width: 1, height: bounds.height).fill()
    }
}
