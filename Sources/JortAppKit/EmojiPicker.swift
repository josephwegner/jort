...
@MainActor
final class EmojiPicker: NSObject, NSPopoverDelegate, NSSearchFieldDelegate,
  NSCollectionViewDataSource, NSCollectionViewDelegate
{
  ...
  init(parent: NSWindow, emoji: String?) {
    ...
    if hadEmoji {
      let clearButton = NSButton(
        title: "Remove Landmark", target: self, action: #selector(removeEmoji))
  ...
  func controlTextDidChange(_ obj: Notification) {
    ...
    visible =
      terms.isEmpty
      ? catalog : catalog.filter { entry in terms.allSatisfy { entry.search.contains($0) } }
  ...
  func collectionView(
    ...
    let item =
      collectionView.makeItem(withIdentifier: EmojiItem.identifier, for: indexPath) as! EmojiItem
  ...
  nonisolated private static func loadCatalog() -> [EmojiEntry] {
    ...
    guard
      let url = bundles.lazy.compactMap({ $0.url(forResource: "emoji", withExtension: "json") })
        .first,
