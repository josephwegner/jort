...
@MainActor open class SettingsPaneViewController: NSViewController {
  ...
  open func resolvePendingChanges(in window: NSWindow, completion: @escaping (Bool) -> Void) {
    completion(true)
  }
}
...
@MainActor public struct SettingsPaneDescriptor {
  ...
  public init(
    id: String, title: String, symbolName: String, keywords: String = "",
...
final class SettingsSidebarController: NSViewController, NSTableViewDataSource, NSTableViewDelegate
{
  ...
  override func loadView() {
    ...
    let column = NSTableColumn(identifier: .init("pane"))
  ...
  func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView?
  {
    ...
    let image = NSImageView(
      image: NSImage(systemSymbolName: pane.symbolName, accessibilityDescription: nil) ?? NSImage())
    let label = NSTextField(labelWithString: pane.title)
    for subview in [image, label] {
  ...
  func tableViewSelectionDidChange(_ notification: Notification) {
    if table.selectedRow >= 0 { onSelect?(table.selectedRow) }
  }
}
...
@MainActor public final class SettingsWindowController: NSWindowController, NSWindowDelegate {
  ...
  public init(panes: [SettingsPaneDescriptor], defaults: UserDefaults = .standard) {
    ...
    window.title = "Jort Settings"
    window.isReleasedWhenClosed = false
    window.minSize = NSSize(width: 680, height: 480)
    window.contentViewController = split
    window.setFrameAutosaveName("JortSettingsWindow")
    super.init(window: window)
    window.delegate = self
    window.setAccessibilityLabel("Jort Settings")
    sidebar.panes = panes
  ...
  public func selectPane(id: String) {
    if let index = panes.firstIndex(where: { $0.id == id }) { requestSelection(index: index) }
  }
  private func requestSelection(index: Int) {
    guard panes.indices.contains(index), panes[index].id != selectedPaneID, !suppressSelection
    else { return }
    guard let window, let active, active.hasUnsavedChanges else {
      select(index: index)
      return
    }
    let oldIndex = panes.firstIndex(where: { $0.id == selectedPaneID }) ?? 0
    suppressSelection = true
    sidebar.table.selectRowIndexes(IndexSet(integer: oldIndex), byExtendingSelection: false)
    suppressSelection = false
    active.resolvePendingChanges(in: window) { [weak self] allowed in
      if allowed { self?.select(index: index) }
    }
  }
  private func select(index: Int) {
    let descriptor = panes[index]
    let controller = instances[descriptor.id] ?? descriptor.makeViewController()
    instances[descriptor.id] = controller
    active = controller
    selectedPaneID = descriptor.id
    defaults.set(descriptor.id, forKey: selectedKey)
    detail.show(controller)
    suppressSelection = true
    sidebar.table.selectRowIndexes(IndexSet(integer: index), byExtendingSelection: false)
    suppressSelection = false
  }
  public func windowShouldClose(_ sender: NSWindow) -> Bool {
    if closingAfterResolution {
      closingAfterResolution = false
      return true
    }
    guard let active, active.hasUnsavedChanges else { return true }
    active.resolvePendingChanges(in: sender) { [weak self] allowed in
      if allowed {
        self?.closingAfterResolution = true
