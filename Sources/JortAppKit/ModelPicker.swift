...
@MainActor
public final class ModelPicker: NSViewController, NSSearchFieldDelegate, NSTableViewDataSource,
  NSTableViewDelegate
{
  ...
  public override func loadView() {
    ...
    let column = NSTableColumn(identifier: .init("model"))
    table.addTableColumn(column)
    table.headerView = nil
    table.rowHeight = 38
    table.delegate = self
    table.dataSource = self
    table.setAccessibilityLabel("Available models")
    search.setAccessibilityLabel("Filter models")
    search.placeholderString = "Filter models"
    search.delegate = self
    table.target = self
    table.action = #selector(selectModel)
    for item in [search, scroll] {
  ...
  public func controlTextDidChange(_ obj: Notification) {
    if search.stringValue.count > 256 {
      search.stringValue = String(search.stringValue.prefix(256))
    }
    models = ModelCatalog.bundled.filter(search.stringValue)
    table.reloadData()
  ...
  public func control(
      ...
      || commandSelector == #selector(NSResponder.moveUp(_:))
    {
      ...
      table.selectRowIndexes(
        IndexSet(integer: max(0, min(models.count - 1, table.selectedRow + step))),
  ...
  @objc public func selectModel() {
    guard models.indices.contains(table.selectedRow) else { return }
    selectedID = models[table.selectedRow].id
    onSelect?(selectedID!)
  }
}
