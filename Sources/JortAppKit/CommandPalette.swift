...
@MainActor public struct PaletteAction {
    ...
    public init(id: String, title: String, keywords: String = "", enabled: @escaping () -> Bool = { true }, execute: @escaping () -> Void) {
        self.id = id; self.title = title; self.keywords = keywords; self.enabled = enabled; self.execute = execute
    }
    public func matches(_ query: String) -> Bool {
        query.split(whereSeparator: \.isWhitespace).allSatisfy {
            (title + " " + keywords).range(of: String($0), options: [.caseInsensitive, .diacriticInsensitive]) != nil
        }
    }
}

@MainActor final class CommandPalette: NSWindowController, NSTableViewDataSource, NSTableViewDelegate, NSSearchFieldDelegate, NSWindowDelegate {
    ...
    init(parent: NSWindow) {
        let panel = NSPanel(contentRect: NSRect(x: 0, y: 0, width: 440, height: 330), styleMask: [.titled, .utilityWindow], backing: .buffered, defer: false)
        panel.title = "Pocket"; panel.isReleasedWhenClosed = false
        panel.appearance = parent.appearance
        super.init(window: panel)
        panel.delegate = self
        panel.setAccessibilityRole(.window); panel.setAccessibilitySubrole(.dialog); panel.setAccessibilityLabel("Pocket")
        let content = NSView(); panel.contentView = content
        query.placeholderString = "Search actions"; query.delegate = self
        ...
        let column = NSTableColumn(identifier: NSUserInterfaceItemIdentifier("action")); table.addTableColumn(column)
        table.headerView = nil; table.rowHeight = 30; table.delegate = self; table.dataSource = self
        table.setAccessibilityLabel("Actions")
        table.target = self; table.action = #selector(executeClickedRow); table.doubleAction = #selector(executeClickedRow)
        table.activate = { [weak self] in self?.executeSelection() }
        table.dismiss = { [weak self] in self?.dismiss() }
        for view in [query, scroll, count] { view.translatesAutoresizingMaskIntoConstraints = false; content.addSubview(view) }
        NSLayoutConstraint.activate([
            query.leadingAnchor.constraint(equalTo: content.leadingAnchor, constant: 16), query.trailingAnchor.constraint(equalTo: content.trailingAnchor, constant: -16), query.topAnchor.constraint(equalTo: content.topAnchor, constant: 16),
            scroll.topAnchor.constraint(equalTo: query.bottomAnchor, constant: 10), scroll.leadingAnchor.constraint(equalTo: query.leadingAnchor), scroll.trailingAnchor.constraint(equalTo: query.trailingAnchor),
            count.topAnchor.constraint(equalTo: scroll.bottomAnchor, constant: 8), count.leadingAnchor.constraint(equalTo: query.leadingAnchor), count.bottomAnchor.constraint(equalTo: content.bottomAnchor, constant: -12)
        ])
    ...
    func present(actions: [PaletteAction], restore: @escaping () -> Void) {
        self.actions = actions; self.restore = restore; reload()
        window?.makeKeyAndOrderFront(nil); window?.makeFirstResponder(query)
    }
    func reload() {
        ...
        table.reloadData()
        if !results.isEmpty { table.selectRowIndexes(IndexSet(integer: results.firstIndex { $0.id == selected } ?? 0), byExtendingSelection: false) }
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        label.setAccessibilityLabel(label.stringValue); label.setAccessibilityEnabled(enabled)
    ...
    private func execute(row: Int) {
        ...
        guard action.enabled() else { reload(); return }
        dismiss(); action.execute()
    }
    ...
    func dismiss() {
        ...
        let callback = restore; restore = nil; callback?()
    }
}
