...
public actor ToolPackageRegistry {
  ...
  public func save(
    ...
    try encoder.encode(package.manifest).write(
      to: directory.appendingPathComponent("tool.json"), options: .atomic)
    try Data((package.manifest.executorType == .model ? package.instructions : package.source).utf8)
      .write(
        to: directory.appendingPathComponent(
          package.manifest.executorType == .model ? "instructions.txt" : "tool.js"),
        options: .atomic)
    var next = index
    next.installed[id] = generation
  ...
  private func commit(_ next: Index) throws {
    try FileManager.default.createDirectory(
      at: installedDirectory, withIntermediateDirectories: true)
    try JSONEncoder().encode(next).write(
      to: installedDirectory.appendingPathComponent("index.json"), options: .atomic)
    index = next
