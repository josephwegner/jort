...
public struct ToolExecutorDispatcher: Sendable {
  ...
  public init(
    ...
    provider: @escaping @Sendable () -> any ModelProvider = {
      FakeModelProvider(.failure(.disconnected))
    },
    ...
    -> ToolExecutionResult
  {
    ...
    } catch is CancellationError { return .init(error: ModelFailure.cancelled.message) } catch {
      return .init(error: ModelFailure.malformed.message)
    }
  }
}
