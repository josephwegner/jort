...
public actor KeychainModelCredentialStore: ModelCredentialStore {
    ...
    public func read() throws -> String? {
        ...
        guard status == errSecSuccess, let data = result as? Data, data.count <= 4096,
              let credential = String(data: data, encoding: .utf8), !credential.isEmpty else { throw ModelFailure.credentialStore }
        return credential
    }
    public func replace(with credential: String) throws {
        guard !credential.isEmpty, credential.utf8.count <= 4096, !credential.contains("\n"), !credential.contains("\r") else { throw ModelFailure.credentialStore }
        let attributes: [String: Any] = [kSecValueData as String: Data(credential.utf8),
            kSecAttrAccessible as String: kSecAttrAccessibleWhenUnlockedThisDeviceOnly]
        let status = SecItemUpdate(query as CFDictionary, attributes as CFDictionary)
        if status == errSecItemNotFound {
            var item = query; attributes.forEach { item[$0] = $1 }
            guard SecItemAdd(item as CFDictionary, nil) == errSecSuccess else { throw ModelFailure.credentialStore }
        } else if status != errSecSuccess { throw ModelFailure.credentialStore }
    }
    public func remove() throws {
        let status = SecItemDelete(query as CFDictionary)
        guard status == errSecSuccess || status == errSecItemNotFound else { throw ModelFailure.credentialStore }
    }
}
...
public actor MemoryModelCredentialStore: ModelCredentialStore {
    ...
    public func replace(with credential: String) throws {
        if failWrites { throw ModelFailure.credentialStore }; self.credential = credential
    }
    public func remove() throws { if failWrites { throw ModelFailure.credentialStore }; credential = nil }
}
