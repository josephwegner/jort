...
public protocol SettingsStore: Sendable {
    ...
    func save(_ definition: UserToolDefinition, expectedRevision: RecordRevision?) async throws -> SettingsSnapshot
...
public struct ToolCatalogBuilder: Sendable {
    ...
    public func configuredTools(templates: [ToolTemplate], snapshot: SettingsSnapshot) -> [ConfiguredTool] {
        ...
        for (_, indexes) in grouped where indexes.count > 1 {
            for index in indexes {
                tools[index].diagnostics.append(.init(severity: .error, field: .catalog, message: "Command name conflicts with another tool."))
            }
        }
        return tools.sorted {
    ...
    private func availableName(base: String, avoiding names: Set<String>) -> String {
        ...
        for suffix in 2...9_999 where !names.contains("\(base)-\(suffix)") { return "\(base)-\(suffix)" }
...
public struct BundledTemplateEnvelope: Codable, Sendable {
    ...
    public init(schemaVersion: Int = 1, templates: [ToolTemplate]) { self.schemaVersion = schemaVersion; self.templates = templates }
}
...
public enum BundledTemplateLoader {
    public static func load(from url: URL?) throws -> [ToolTemplate] {
        ...
        guard data.count <= SettingsLimits.maximumDecodedBytes else { throw SettingsStoreError.sizeLimit }
        let envelope = try JSONDecoder().decode(BundledTemplateEnvelope.self, from: data)
        guard envelope.schemaVersion == 1, envelope.templates.count <= SettingsLimits.maximumTemplates else { throw SettingsStoreError.unsupportedVersion }
        let ids = Set(envelope.templates.map(\.id)), names = Set(envelope.templates.map { SettingsValidation.normalizedCommandName($0.commandName) })
        guard ids.count == envelope.templates.count, names.count == envelope.templates.count,
              envelope.templates.allSatisfy({ SettingsValidation.diagnostics(for: $0).isEmpty }) else { throw SettingsStoreError.invalidData }
