...
public enum ToolDiagnosticField: String, Codable, Sendable {
  case displayName, commandName, summary, source, catalog
}
...
public struct ToolDiagnostic: Codable, Equatable, Sendable, Identifiable {
  ...
  public init(
    id: UUID = UUID(), severity: ToolDiagnosticSeverity, field: ToolDiagnosticField,
...
public struct ToolTemplate: Codable, Equatable, Sendable, Identifiable {
  ...
  public init(
    id: ToolID, version: Int = 1, displayName: String, commandName: String,
...
public struct UserToolDefinition: Codable, Equatable, Sendable, Identifiable {
  ...
  public init(
    id: ToolID = ToolID("user.jort.tool-" + UUID().uuidString.lowercased()),
    revision: RecordRevision = RecordRevision(),
    basedOnTemplateID: ToolID? = nil, displayName: String, commandName: String,
    summary: String = "", source: String, isEnabled: Bool = false
  ) {
    self.id = id
    self.revision = revision
    self.basedOnTemplateID = basedOnTemplateID
    self.displayName = displayName
    self.commandName = commandName
    self.summary = summary
    self.source = source
...
public struct ToolDraft: Equatable, Sendable {
  ...
  public init(
    definition: UserToolDefinition, baseRevision: RecordRevision? = nil,
    original: UserToolDefinition? = nil
  ) {
  ...
  public static func new() -> ToolDraft {
    ToolDraft(
      definition: UserToolDefinition(
        displayName: "Untitled Tool", commandName: "untitled-tool", source: ""))
  }
}
...
public enum SettingsValidation {
  ...
  public static func diagnostics(
    for definition: UserToolDefinition, occupiedNames: Set<String> = []
  ) -> [ToolDiagnostic] {
    ...
    if display.isEmpty {
      diagnostics.append(
        .init(severity: .error, field: .displayName, message: "Display name is required."))
    }
    if display.utf8.count > SettingsLimits.maximumNameBytes {
      diagnostics.append(
        .init(severity: .error, field: .displayName, message: "Display name is too long."))
    }
    let commandScalars = command.unicodeScalars
    let validCommand =
      !command.isEmpty && command.utf8.count <= 64
      && commandScalars.first.map {
        (97...122).contains(Int($0.value))
      } == true
      && commandScalars.allSatisfy {
        (97...122).contains(Int($0.value)) || (48...57).contains(Int($0.value)) || $0.value == 45
      }
    if !validCommand {
      diagnostics.append(
        .init(
          severity: .error, field: .commandName,
          message: "Use 1–64 lowercase letters, numbers, or hyphens, beginning with a letter."))
    }
    if occupiedNames.contains(command) {
      diagnostics.append(
        .init(
          severity: .error, field: .commandName, message: "That command name is already in use."))
    }
    if definition.summary.utf8.count > SettingsLimits.maximumSummaryBytes {
      diagnostics.append(
        .init(severity: .error, field: .summary, message: "Description is too long."))
    }
    if let manifest = definition.manifest, (try? manifest.validate()) == nil {
      diagnostics.append(
        .init(severity: .error, field: .catalog, message: "Unsupported or invalid tool contract."))
    }
    if definition.manifest?.executorType == .model {
      if (definition.instructions ?? "").trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        || (definition.instructions?.utf8.count ?? 0) > 32_768
        || (definition.instructions?.contains("\0") ?? false)
      {
        diagnostics.append(
          .init(
            severity: .error, field: .source,
            message: "Enter instructions within 32 KiB, without NUL characters."))
      }
      if ModelCatalog.bundled.model(id: definition.manifest?.modelID ?? "") == nil {
        diagnostics.append(
          .init(
            severity: .error, field: .catalog,
            message: "Selected model is unavailable. Choose a model in Tools Settings."))
      }
      return diagnostics
    }
    if definition.source.utf8.count > SettingsLimits.maximumSourceBytes {
      diagnostics.append(
        .init(severity: .error, field: .source, message: "JavaScript source exceeds 256 KiB."))
    }
    if definition.source.unicodeScalars.contains(where: { $0.value == 0 }) {
      diagnostics.append(
        .init(
          severity: .error, field: .source,
          message: "JavaScript source cannot contain NUL characters."))
    }
    return diagnostics
  }
