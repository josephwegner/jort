...
public actor PackageSettingsStore: SettingsStore {
    ...
    public init(registry: ToolPackageRegistry, preferences: SQLiteSettingsStore) {
        self.registry = registry; self.preferences = preferences
    }
    public func load() async throws -> SettingsSnapshot {
        ...
        legacyTools = legacy.customTools.map { original in
            var value = original
            if original.id.rawValue.range(of: #"^[a-z][a-z0-9-]*(\.[a-z][a-z0-9-]*)+$"#, options: .regularExpression) == nil {
                let hash = SHA256.hash(data: Data(original.id.rawValue.utf8)).prefix(16).map { String(format: "%02x", $0) }.joined()
    ...
    public func updates() -> AsyncStream<SettingsSnapshot> {
        ...
        return AsyncStream { continuation in
            streams[id] = continuation; continuation.yield(snapshot)
    ...
    public func save(_ definition: UserToolDefinition, expectedRevision: RecordRevision?) async throws -> SettingsSnapshot {
        let current = snapshot.customTools.first { $0.id == definition.id }
        guard current?.revision == expectedRevision else { throw SettingsStoreError.conflict(current: current?.revision) }
        var manifest = definition.manifest ?? ToolManifest(id: definition.id.rawValue, name: definition.displayName, command: "/" + SettingsValidation.normalizedCommandName(definition.commandName))
        ...
        if let originalID = legacyIDs[definition.id], let revision = current?.revision {
            ...
            legacyIDs.removeValue(forKey: definition.id); legacyTools.removeAll { $0.id == definition.id }
        }
        return try await refresh()
    }
    public func delete(id: ToolID, expectedRevision: RecordRevision) async throws -> SettingsSnapshot {
        guard snapshot.customTools.first(where: { $0.id == id })?.revision == expectedRevision else { throw SettingsStoreError.conflict(current: snapshot.customTools.first(where: { $0.id == id })?.revision) }
        if let originalID = legacyIDs[id] {
            _ = try await preferences.delete(id: originalID, expectedRevision: expectedRevision)
            legacyIDs.removeValue(forKey: id); legacyTools.removeAll { $0.id == id }
        } else { _ = try await registry.remove(id: id.rawValue) }
        return try await refresh()
    }
    public func close() async throws {
        streams.values.forEach { $0.finish() }; streams = [:]
    ...
    private func refresh(preferences: [String: String]? = nil) async throws -> SettingsSnapshot {
        ...
        var value = SettingsSnapshot(revision: CatalogRevision(snapshot.revision.rawValue + 1), preferences: preferences ?? snapshot.preferences)
        for entry in registry.packages {
            if entry.isBundled { value.templateOverrides[ToolID(entry.id)] = entry.isEnabled }
            else {
        ...
        snapshot = value; streams.values.forEach { $0.yield(value) }; return value
    }
}
