...
public struct BoundedOpenRouterTransport: OpenRouterTransport {
    ...
    public func send(_ request: URLRequest, maximumBytes: Int) async throws -> Data {
        guard request.url?.scheme == "https", request.url?.host == "openrouter.ai",
              ...
              ["/api/v1/chat/completions", "/api/v1/auth/keys", "/api/v1/key"].contains(request.url?.path ?? ""),
              (1...8_388_608).contains(maximumBytes) else { throw ModelFailure.malformed }
        ...
        let session = URLSession(configuration: config, delegate: FixedOriginDelegate(), delegateQueue: nil)
        ...
        do {
            ...
            return data
        } catch let failure as ModelFailure { throw failure }
        catch is CancellationError { throw ModelFailure.cancelled }
...
public struct OpenRouterProvider: ModelProvider {
    ...
    public init(credentials: any ModelCredentialStore, transport: any OpenRouterTransport = BoundedOpenRouterTransport()) {
    ...
    public func execute(_ request: ModelRequest) async throws -> String {
        ...
        let data = try await transport.send(http, maximumBytes: min(8_388_608, request.maximumBytes * 6 + 65_536))
    ...
    public static func decode(_ data: Data, request: ModelRequest) throws -> String {
        guard data.count <= min(8_388_608, request.maximumBytes * 6 + 65_536),
              ...
              let choice = choices.first, choice["finish_reason"] as? String == "stop", choice["delta"] == nil,
              let message = choice["message"] as? [String: Any], noToolCalls(message["tool_calls"]),
              message["function_call"] == nil || message["function_call"] is NSNull, let output = message["content"] as? String,
              let usage = value["usage"] as? [String: Any], let tokens = usage["completion_tokens"] as? Int,
              (0...request.maximumTokens).contains(tokens) else { throw ModelFailure.malformed }
