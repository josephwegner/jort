...
public struct ToolExecutionResult: Codable, Equatable, Sendable {
    ...
    public init(output: String? = nil, error: String? = nil) { self.output = output; self.error = error }
}
...
private final class ToolCancellation: @unchecked Sendable {
    ...
    init?() { guard let pointer = jort_js_cancellation_new() else { return nil }; self.pointer = pointer }
...
public enum ToolRuntime {
    ...
    public static func execute(_ package: ToolPackage, input: ToolExecutionInput,
        ...
        return await withTaskCancellationHandler {
            await Task.detached {
                var status: Int32 = 1
                guard let bytes = jort_js_run(package.source, json, validationOnly ? "validate" : "default", 16 * 1024 * 1024,
                    min(30, max(0.01, timeout)), package.manifest.maximumOutputBytes * 6 + 4096,
                    token.pointer, &status) else { return ToolExecutionResult(error: "Runtime allocation failed.") }
                defer { jort_js_free(bytes) }
                if status == 2 { return .init(error: "Cancelled.") }
                if status == 3 { return .init(error: "Tool timed out.") }
                guard status == 0, let result = try? JSONDecoder().decode(ToolExecutionResult.self,
                    from: Data(String(cString: bytes).utf8)), (result.output == nil) != (result.error == nil) else {
                ...
                guard output.utf8.count <= package.manifest.maximumOutputBytes,
                      lines <= package.manifest.maximumOutputLines else {
