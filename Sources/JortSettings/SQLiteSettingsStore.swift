...
public actor SQLiteSettingsStore: SettingsStore, ToolCatalog {
    ...
    public init(directory: URL, templates: [ToolTemplate] = [],
                inject: @escaping @Sendable (SettingsStoreStage) throws -> Void = { _ in }) {
        self.directory = directory; self.templates = templates; self.inject = inject
    }
    ...
    public func setPreference(key: String, value: String?) throws -> SettingsSnapshot {
        guard !key.isEmpty, key.utf8.count <= SettingsLimits.maximumNameBytes,
              value?.utf8.count ?? 0 <= SettingsLimits.maximumSummaryBytes else { throw SettingsStoreError.sizeLimit }
    ...
    public func save(_ definition: UserToolDefinition, expectedRevision: RecordRevision?) throws -> SettingsSnapshot {
        var definition = definition
        definition.commandName = SettingsValidation.normalizedCommandName(definition.commandName)
        let occupied = Set(templates.map { SettingsValidation.normalizedCommandName($0.commandName) } +
            snapshot.customTools.filter { $0.id != definition.id }.map { SettingsValidation.normalizedCommandName($0.commandName) })
        guard SettingsValidation.diagnostics(for: definition, occupiedNames: occupied).isEmpty,
              snapshot.customTools.count < SettingsLimits.maximumCustomTools || snapshot.customTools.contains(where: { $0.id == definition.id }) else {
    ...
    private func mutate(_ body: (SettingsConnection) throws -> Void) throws -> SettingsSnapshot {
        guard let connection else { throw SettingsStoreError.unavailable("Settings have not loaded safely.") }
        try inject(.write)
        do {
    ...
    private func preserveForMigration(_ url: URL) throws {
        ...
        for suffix in ["-wal", "-shm"] {
            ...
            if FileManager.default.fileExists(atPath: companion.path) {
                try FileManager.default.copyItem(at: companion, to: URL(fileURLWithPath: copy.path + suffix))
            }
        }
    }
    ...
    private static func message(for error: Error) -> String {
        switch error {
        case SettingsStoreError.unsupportedVersion: return "These settings were created by a newer version of Jort."
        case SettingsStoreError.invalidData: return "Settings contain invalid data and were preserved unchanged."
...
private final class SettingsConnection {
    ...
    static func inspectVersion(url: URL) throws -> Int {
        ...
        guard result == SQLITE_OK, let handle else {
            let message = handle.map { String(cString: sqlite3_errmsg($0)) } ?? "Unable to inspect settings"
            sqlite3_close_v2(handle); throw SettingsStoreError.unavailable(message)
        }
        defer { sqlite3_close_v2(handle) }
        var statement: OpaquePointer?
        guard sqlite3_prepare_v2(handle, "PRAGMA user_version", -1, &statement, nil) == SQLITE_OK else { throw SettingsStoreError.invalidData }
    ...
    static func isRecognizedLegacyV0(url: URL) throws -> Bool {
        ...
        guard sqlite3_open_v2(url.path, &handle, SQLITE_OPEN_READONLY | SQLITE_OPEN_FULLMUTEX, nil) == SQLITE_OK, let handle else {
            sqlite3_close_v2(handle); throw SettingsStoreError.invalidData
        }
        defer { sqlite3_close_v2(handle) }
        var statement: OpaquePointer?
        guard sqlite3_prepare_v2(handle, "SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name", -1, &statement, nil) == SQLITE_OK else { throw SettingsStoreError.invalidData }
        defer { sqlite3_finalize(statement) }
        var names: [String] = [], step = sqlite3_step(statement)
        while step == SQLITE_ROW {
            guard let text = sqlite3_column_text(statement, 0) else { return false }
            names.append(String(cString: text)); step = sqlite3_step(statement)
        }
    ...
    func close() throws { guard let db else { return }; try check(sqlite3_close(db)); self.db = nil }
    func failure(_ code: Int32) -> SettingsStoreError { .unavailable(db.map { String(cString: sqlite3_errmsg($0)) } ?? "SQLite closed (\(code))") }
    ...
    func prepare(_ sql: String) throws -> OpaquePointer {
        var statement: OpaquePointer?; try check(sqlite3_prepare_v2(db, sql, -1, &statement, nil))
        guard let statement else { throw SettingsStoreError.invalidData }; return statement
    }
    func version() throws -> Int {
        let statement = try prepare("PRAGMA user_version"); defer { sqlite3_finalize(statement) }
        guard sqlite3_step(statement) == SQLITE_ROW else { throw SettingsStoreError.invalidData }
        return Int(sqlite3_column_int(statement, 0))
    }
    ...
    func transaction(_ body: () throws -> Void) throws {
        ...
        catch { let primary = error; try? execute("ROLLBACK"); throw primary }
    }
    func bind(_ value: String?, to statement: OpaquePointer, at index: Int32) throws {
        if let value { try check(sqlite3_bind_text(statement, index, value, -1, transient)) }
        else { try check(sqlite3_bind_null(statement, index)) }
    }
    func text(_ statement: OpaquePointer, _ column: Int32) throws -> String {
        guard let text = sqlite3_column_text(statement, column) else { throw SettingsStoreError.invalidData }
        return String(cString: text)
    }
    func setPreference(key: String, value: String?) throws {
        let sql = value == nil ? "DELETE FROM preferences WHERE key=?" : "INSERT INTO preferences(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value"
        let statement = try prepare(sql); defer { sqlite3_finalize(statement) }
        try bind(key, to: statement, at: 1); if let value { try bind(value, to: statement, at: 2) }
    ...
    func setTemplateEnabled(id: ToolID, enabled: Bool?) throws {
        let sql = enabled == nil ? "DELETE FROM template_overrides WHERE id=?" : "INSERT INTO template_overrides(id,enabled) VALUES(?,?) ON CONFLICT(id) DO UPDATE SET enabled=excluded.enabled"
        let statement = try prepare(sql); defer { sqlite3_finalize(statement) }
        try bind(id.rawValue, to: statement, at: 1); if let enabled { try check(sqlite3_bind_int(statement, 2, enabled ? 1 : 0)) }
        guard sqlite3_step(statement) == SQLITE_DONE else { throw failure(sqlite3_errcode(db)) }
    }
    ...
    func delete(id: ToolID, expectedRevision: RecordRevision) throws {
        guard try recordRevision(id: id) == expectedRevision else { throw SettingsStoreError.conflict(current: try recordRevision(id: id)) }
        let statement = try prepare("DELETE FROM custom_tools WHERE id=? AND revision=?"); defer { sqlite3_finalize(statement) }
        try bind(id.rawValue, to: statement, at: 1); try check(sqlite3_bind_int64(statement, 2, expectedRevision.rawValue))
        guard sqlite3_step(statement) == SQLITE_DONE, sqlite3_changes(db) == 1 else { throw SettingsStoreError.conflict(current: try recordRevision(id: id)) }
    }
    func recordRevision(id: ToolID) throws -> RecordRevision? {
        let statement = try prepare("SELECT revision FROM custom_tools WHERE id=?"); defer { sqlite3_finalize(statement) }
        try bind(id.rawValue, to: statement, at: 1); let step = sqlite3_step(statement)
        if step == SQLITE_DONE { return nil }; guard step == SQLITE_ROW else { throw failure(step) }
        return RecordRevision(sqlite3_column_int64(statement, 0))
    }
    func advanceCatalogRevision() throws { try execute("UPDATE settings_meta SET catalog_revision=catalog_revision+1 WHERE id=1") }
    func readSnapshot(availability: SettingsAvailability) throws -> SettingsSnapshot {
        let meta = try prepare("SELECT catalog_revision FROM settings_meta WHERE id=1"); defer { sqlite3_finalize(meta) }
        ...
        let preferenceRows = try prepare("SELECT key,value FROM preferences ORDER BY key"); defer { sqlite3_finalize(preferenceRows) }
        var step = sqlite3_step(preferenceRows)
        while step == SQLITE_ROW { preferences[try text(preferenceRows, 0)] = try text(preferenceRows, 1); step = sqlite3_step(preferenceRows) }
        guard step == SQLITE_DONE else { throw failure(step) }
        var overrides: [ToolID: Bool] = [:]
        let overrideRows = try prepare("SELECT id,enabled FROM template_overrides ORDER BY id"); defer { sqlite3_finalize(overrideRows) }
        ...
        guard step == SQLITE_DONE else { throw failure(step) }
        return SettingsSnapshot(revision: revision, preferences: preferences, templateOverrides: overrides, customTools: tools, availability: availability)
    }
}
