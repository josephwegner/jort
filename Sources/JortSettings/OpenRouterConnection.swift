...
public struct PKCEAttempt: Sendable {
  ...
  public init(now: Date = Date()) throws {
    func random() throws -> String {
      ...
      guard SecRandomCopyBytes(kSecRandomDefault, bytes.count, &bytes) == errSecSuccess else {
        throw ModelFailure.authorization
      }
  ...
  public func authorizationURL(port: UInt16) -> URL {
    ...
    url.queryItems = [
      .init(name: "callback_url", value: "http://localhost:\(port)/callback/\(nonce)"),
  ...
  public func code(from target: String, now: Date = Date()) throws -> String {
    guard now < expiresAt, target.utf8.count <= 8192, target.hasPrefix("/callback/\(nonce)?"),
      ...
      !code.contains("\0")
    else { throw ModelFailure.authorization }
    return code
  }
}
  ...
  public func receive(_ attempt: PKCEAttempt) async throws -> String {
    ...
    let job = Task.detached { [self] in
      ...
      while Date() < attempt.expiresAt {
        ...
        var data = Data()
        let deadline = min(attempt.expiresAt, Date().addingTimeInterval(2))
        ...
        guard parts.count == 3, parts[0] == "GET", parts[2] == "HTTP/1.1",
          let code = try? attempt.code(from: String(parts[1]))
        else { continue }
        let response =
...
public actor OpenRouterConnection {
  ...
  public func connect(openBrowser: @escaping @Sendable (URL) async -> Bool) async throws {
    ...
    defer { busy = false }
    do {
      let attempt = try PKCEAttempt()
      let callback = try LoopbackOAuthCallback()
      guard await openBrowser(attempt.authorizationURL(port: callback.port)) else {
        throw ModelFailure.authorization
      }
  ...
  func exchange(code: String, attempt: PKCEAttempt) async throws {
    guard Date() < attempt.expiresAt, !code.isEmpty, code.utf8.count <= 4096 else {
      throw ModelFailure.authorization
    }
    usedAttempts = usedAttempts.filter { $0.value > Date() }
    guard usedAttempts[attempt.nonce] == nil, usedAttempts.count < 64 else {
      throw ModelFailure.authorization
    }
    ...
    request.httpBody = try JSONSerialization.data(withJSONObject: [
      "code": code, "code_verifier": attempt.verifier, "code_challenge_method": "S256",
    ])
  ...
  private func verify(_ key: String) async throws -> ModelConnectionStatus {
    ...
    guard data.count <= 16_384,
      let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
      let metadata = object["data"] as? [String: Any], metadata["label"] is String
    else { throw ModelFailure.malformed }
    // Never persist server strings: even a compromised response cannot reflect a key.
    let expiration = (metadata["expires_at"] as? String).flatMap {
