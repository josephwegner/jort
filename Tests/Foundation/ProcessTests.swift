...
final class ProcessTests: StoreTestCase {
  private var executable: URL {
    Bundle(for: Self.self).bundleURL.deletingLastPathComponent().appendingPathComponent(
      "StoreLockProbe")
  }
  private func launch(_ root: URL, mode: String) throws -> (Process, Pipe) {
    let process = Process(), pipe = Pipe()
    process.executableURL = executable
    process.arguments = [root.path, mode]
    process.standardOutput = pipe
    try process.run()
    removeAfterStoresClose(root)
    storeResources.prepare {
  ...
  func testTwoProcessesRaceAndCrashReleasesLock() throws {
    ...
    XCTAssertTrue(
      String(decoding: output.fileHandleForReading.availableData, as: UTF8.self).contains("OWNED"))
    XCTAssertTrue(
      FileManager.default.fileExists(
        atPath: root.appendingPathComponent("Store/Jort.sqlite-wal").path))
    ...
    XCTAssertEqual(duplicate.terminationStatus, 73)
    ...
    afterCrash.waitUntilExit()
    XCTAssertEqual(afterCrash.terminationStatus, 0)
    let (reopened, reopenedOutput) = try launch(root, mode: "store")
    XCTAssertTrue(
      String(decoding: reopenedOutput.fileHandleForReading.availableData, as: UTF8.self).contains(
        "OWNED"))
    let checkpoint = try RecoveryCheckpoints(
      directory: root.appendingPathComponent("Store"), inject: { _ in }
    ).recover()
    XCTAssertEqual(checkpoint.text, "process save")
    kill(reopened.processIdentifier, SIGKILL)
    reopened.waitUntilExit()
  }
  func testDifferentStoresHaveIndependentProcessOwners() throws {
    ...
    a.waitUntilExit()
    b.waitUntilExit()
    XCTAssertEqual(a.terminationStatus, 0)
    XCTAssertEqual(b.terminationStatus, 0)
  }
