...
final class MigrationTests: XCTestCase {
    ...
    func testPrototypeLandmarksSurviveMigration() async throws {
        ...
        let jsonURL = try XCTUnwrap(Bundle(for: Self.self).url(forResource: "v2", withExtension: "json"))
        ...
        document["landmarks"] = [
            ["id": "22222222-2222-2222-2222-222222222222", "lineID": "11111111-1111-1111-1111-111111111111", "emoji": "🌲"],
            ["id": "33333333-3333-3333-3333-333333333333", "lineID": "44444444-4444-4444-4444-444444444444", "emoji": "🦊"]
        ...
        let store = SQLiteStore(directory: root), expected = try PersistenceFormat.decode(data).snapshot
        let actual = try await store.load()
        XCTAssertEqual(actual, expected); XCTAssertEqual(actual.landmarks.count, 2)
        XCTAssertEqual(actual.landmarks.filter(\.detached).count, 1)
        XCTAssertEqual(try Data(contentsOf: canonical), original)
        try await store.close()
        let reopened = SQLiteStore(directory: root)
        let restored = try await reopened.load(); XCTAssertEqual(restored, expected)
        try await reopened.close()
    }
