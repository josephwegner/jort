...
final class DocumentSearchTests: XCTestCase {
  ...
  @MainActor func testLiteralCaseAndUnicodeWordBoundaries() throws {
    ...
    XCTAssertEqual(
      try DocumentSearch.scan(
        snapshot, query: "cat", options: .init(caseSensitive: true, wholeWord: true)
      ).matches.count, 1)
    XCTAssertEqual(
      try DocumentSearch.scan(snapshot, query: "cat", options: .init(wholeWord: true)).matches
        .count, 2)
    XCTAssertEqual(
      try DocumentSearch.scan(snapshot, query: "cafe", options: .init(wholeWord: true)).matches
        .count, 0)
    ...
    for query in ["👩🏽‍💻", "cafe\u{301}", "café"] {
      ...
      XCTAssertEqual(
        (snapshot.text as NSString).substring(with: try XCTUnwrap(match.resolve(in: snapshot))),
        query)
    }
  }
  @MainActor func testMultilineAndShiftedAnchorsRejectChangedTextAndDocuments() throws {
    ...
    try document.apply(
      .init(
        baseRevision: document.snapshot.revision, origin: .native,
        mutation: .edit(text: "longer header\nneedle\nnext", range: nil, replacementLength: nil)))
  ...
  @MainActor func testEmptyLimitAndBoundedContext() throws {
    ...
    XCTAssertEqual(page.matches.count, 10)
    XCTAssertTrue(page.hasMore)
    XCTAssertTrue(page.matches.allSatisfy { $0.snippet.utf16.count < 200 })
    let combining = try owner("needle " + "e" + String(repeating: "\u{301}", count: 10_000))
      .snapshot
    XCTAssertLessThan(
      try XCTUnwrap(DocumentSearch.scan(combining, query: "needle").matches.first).snippet.utf16
        .count, 200)
  }
