...
final class StorageTests: StoreTestCase {
  ...
  private func snapshot() -> DocumentSnapshot {
    DocumentSnapshot(
      ...
      lines: [
        LineMeta(
          location: 0, length: 11, createdAt: Date(timeIntervalSinceReferenceDate: 100),
          lastEditedAt: Date(timeIntervalSinceReferenceDate: 100)),
        LineMeta(location: 11, length: 0),
      ])
  }
  func testRoundTripAndRecoveryEveryStage() async throws {
    for stage in [StoreStage.backup, .create, .write, .validate, .replace, .close] {
      let root = try temp()
      let original = ownStore(SQLiteStore(directory: root))
  ...
  func testSnapshotAndBindFailureDoNotClaimSuccess() async throws {
    for stage in [StoreStage.bind, .snapshot] {
      let root = try temp(), normal = ownStore(SQLiteStore(directory: try temp()))
      _ = try await normal.load()
      try await normal.close()
      let initial = ownStore(SQLiteStore(directory: root))
      ...
      do {
        _ = try await broken.save(snapshot())
        XCTFail("Must fail")
      } catch {}
      try await broken.close()
      let reopened = ownStore(SQLiteStore(directory: root))
  ...
  func testSingleOwnerAndDifferentDirectories() async throws {
    let root = try temp(), first = ownStore(SQLiteStore(directory: try temp()))
    let a = ownStore(SQLiteStore(directory: root)), b = ownStore(SQLiteStore(directory: root))
    _ = try await a.load()
    _ = try await first.load()
    do {
      _ = try await b.load()
      XCTFail("Second owner")
    } catch { XCTAssertEqual(error as? StoreError, .ownership) }
    do {
      _ = try await b.recover()
      XCTFail("Second recovery owner")
    } catch { XCTAssertEqual(error as? StoreError, .ownership) }
    try await a.close()
    _ = try await b.load()
    try await b.close()
  ...
  func testFormatVersionsAndSize() throws {
    ...
    XCTAssertThrowsError(try PersistenceFormat.decode(Data("{\"formatVersion\":99}".utf8))) {
      XCTAssertEqual($0 as? StoreError, .unsupportedVersion)
    }
    XCTAssertThrowsError(
      try PersistenceFormat.decode(Data(repeating: 0, count: PersistenceFormat.maximumBytes + 1))
    ) { XCTAssertEqual($0 as? StoreError, .sizeLimit) }
    var json = try XCTUnwrap(JSONSerialization.jsonObject(with: data) as? [String: Any])
    var document = try XCTUnwrap(json["document"] as? [String: Any])
    document.removeValue(forKey: "landmarks")
    json["document"] = document
    XCTAssertThrowsError(try PersistenceFormat.decode(JSONSerialization.data(withJSONObject: json)))
    json["formatVersion"] = 2
    XCTAssertEqual(
      try PersistenceFormat.decode(JSONSerialization.data(withJSONObject: json)).snapshot.landmarks,
      [])
  }
  func testBusyAndReadOnlyFailuresPreserveRetryableStore() async throws {
    let root = try temp(), store = ownStore(SQLiteStore(directory: root))
    _ = try await store.load()
    var external: OpaquePointer?
    XCTAssertEqual(
      sqlite3_open(root.appendingPathComponent("Store/Jort.sqlite").path, &external), SQLITE_OK)
    defer {
      if let external { XCTAssertEqual(sqlite3_close(external), SQLITE_OK) }
    }
  ...
  func testOversizedSaveKeepsLastCommittedState() async throws {
    ...
    let date = Date()
    let huge = DocumentSnapshot(
      text: String(repeating: "x", count: count), revision: 1,
      lines: [LineMeta(location: 0, length: count, createdAt: date, lastEditedAt: date)])
    do {
      _ = try await store.save(huge)
