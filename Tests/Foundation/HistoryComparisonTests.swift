  ...
  @MainActor func testToolMetadataOnlyChangesRemainVisibleAndCanonical() throws {
    ...
    try owner.apply(
      .init(
        ...
        mutation: .edit(text: "/calc 3+36", range: nil, replacementLength: nil)))
    ...
    let annotated = DocumentSnapshot(
    ...
    XCTAssertEqual(
      diff.lines.first { $0.kind == .added }?.tools.first?.label, "/calc · Pending merge")
  ...
  @MainActor func testChangesReconstructBothSnapshotsIncludingUnicodeAndLandmarks() throws {
    ...
    try owner.apply(
      .init(
        ...
        mutation: .landmark(Landmark(lineID: owner.snapshot.lines[2].id, emoji: "🌲"))))
    ...
    XCTAssertEqual(
      diff.lines.filter { $0.kind != .removed }.map(\.text).joined(), owner.snapshot.text)
  ...
  func testLargeReorderedDocumentRemainsReconstructable() throws {
    ...
    func snapshot(_ order: [Int]) -> DocumentSnapshot {
      ...
      for (index, value) in order.enumerated() {
        ...
        lines.append(
          LineMeta(
            id: ids[value], location: text.utf16.count, length: line.utf16.count, createdAt: date,
            lastEditedAt: date))
        text += line
      }
