...
final class DocumentTests: XCTestCase {
  @MainActor func testTransactionsRevisionAndStaleAnchor() throws {
    ...
    let first = try owner.apply(
      .init(
        ...
        mutation: .edit(text: "hello\nworld", range: nil, replacementLength: nil)))
    XCTAssertEqual(first.after.revision, 1)
    let stale = DocumentTransaction(
      baseRevision: 1, origin: .automation,
      mutation: .insertAfter(lineID: first.after.lines[0].id, text: "output"))
    _ = try owner.apply(
      .init(
    ...
    let undo = try owner.apply(
      .init(baseRevision: 2, origin: .undo, undoPolicy: .replay, mutation: .restore(first.before)))
  ...
  @MainActor func testLineageAndLandmarkPrototype() throws {
    ...
    func edit(_ text: String, _ range: NSRange? = nil, _ length: Int? = nil) throws {
      try owner.apply(
        .init(
          ...
          mutation: .edit(text: text, range: range, replacementLength: length)))
    }
    try edit("first\nsecond\nthird")
    let original = owner.snapshot
    let landmark = Landmark(lineID: original.lines[1].id, emoji: "🌲")
    try owner.apply(
      .init(baseRevision: owner.snapshot.revision, origin: .metadata, mutation: .landmark(landmark))
    )
    let duplicate = Landmark(lineID: original.lines[2].id, emoji: "🌲")
    try owner.apply(
      .init(
        baseRevision: owner.snapshot.revision, origin: .metadata, mutation: .landmark(duplicate)))
    ...
    try owner.apply(
      .init(baseRevision: owner.snapshot.revision, origin: .undo, mutation: .restore(withAnchor)))
    XCTAssertFalse(owner.snapshot.isDetached(landmark))
    let changed = Landmark(id: landmark.id, lineID: landmark.lineID, emoji: "🦊")
    try owner.apply(
      .init(baseRevision: owner.snapshot.revision, origin: .metadata, mutation: .landmark(changed)))
    XCTAssertEqual(
      owner.snapshot.landmarks.first(where: { $0.id == landmark.id })?.lineID, landmark.lineID)
  ...
  @MainActor func testRandomizedTransactionsUndoRedo() throws {
    ...
    for _ in 0..<500 {
      ...
      characters.replaceSubrange(
        start..<(start + count), with: Array(["x", "\n", "🦊", " ", "é", "\r\n"][next(6)]))
      let result = try owner.apply(
        .init(
          baseRevision: before.revision, origin: .native,
          mutation: .edit(text: String(characters), range: nil, replacementLength: nil)))
  ...
  func testInvalidMetadataRejected() throws {
    ...
    for snapshot in [
      ...
      DocumentSnapshot(
        text: "x", lines: [.init(location: 0, length: 2, createdAt: date, lastEditedAt: date)]),
      DocumentSnapshot(
        text: "x\ny",
        lines: [
          .init(id: id, location: 0, length: 2, createdAt: date, lastEditedAt: date),
          .init(id: id, location: 2, length: 1, createdAt: date, lastEditedAt: date),
        ]),
      DocumentSnapshot(
        text: " ", lines: [.init(location: 0, length: 1, createdAt: date, lastEditedAt: date)]),
