...
final class HistoryStorageTests: XCTestCase {
    func testIdleDefaultUpgradesLegacyPolicyAndPreservesCustomIntervals() throws {
        ...
        XCTAssertEqual(try JSONDecoder().decode(HistorySettings.self, from: JSONEncoder().encode(custom)), custom)
        XCTAssertEqual(HistorySettings().idleInterval, 60)
    }
    private let date = Date(timeIntervalSinceReferenceDate: 100)

    private func root() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("HistoryTests-\(UUID())")
        try FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
        addTeardownBlock { try FileManager.default.removeItem(at: url) }
        return url
    }

    private func snapshot(documentID: UUID = UUID(), text: String = "Hello 🦊", generation: Int64 = 7) -> DocumentSnapshot {
        let line = LineMeta(location: 0, length: text.utf16.count, createdAt: date, lastEditedAt: date)
        return DocumentSnapshot(documentID: documentID, text: text, revision: generation, lines: [line],
            landmarks: [Landmark(lineID: line.id, emoji: "🌲"), Landmark(lineID: UUID(), emoji: "🦊", detached: true)])
    }

    private func execute(_ root: URL, _ sql: String) throws {
        var db: OpaquePointer?
        guard sqlite3_open(root.appendingPathComponent("Store/Jort.sqlite").path, &db) == SQLITE_OK else {
    ...
    func testEnvelopeRoundTripPreservesEveryIdentityAndMetadata() throws {
        ...
        let (data, metadata) = try HistoryRevisionFormat.encode(value, reason: "Landmark changed", timestamp: date)
    ...
    func testStateHashExcludesOnlyGeneration() throws {
        let value = snapshot()
        let newer = DocumentSnapshot(documentID: value.documentID, text: value.text, revision: 100,
            lines: value.lines, landmarks: value.landmarks)
        XCTAssertEqual(try HistoryRevisionFormat.stateHash(value), try HistoryRevisionFormat.stateHash(newer))
        let changedMetadata = DocumentSnapshot(documentID: value.documentID, text: value.text,
            revision: value.revision, lines: value.lines, landmarks: [])
        XCTAssertNotEqual(try HistoryRevisionFormat.stateHash(value), try HistoryRevisionFormat.stateHash(changedMetadata))
        XCTAssertNotEqual(try HistoryRevisionFormat.stateHash(value), try HistoryRevisionFormat.stateHash(snapshot()))
    }
    ...
    func testEnvelopeRefusesUnsupportedMalformedAndOversizedData() throws {
        ...
        XCTAssertThrowsError(try HistoryRevisionFormat.encode(value, reason: String(repeating: "x", count: 257), timestamp: date))
        let invalid = DocumentSnapshot(documentID: value.documentID, text: value.text, revision: 1,
    ...
    func testEnvelopeDetectsCorruptionAndBoundsDeclaredDecompression() throws {
        ...
        for field in ["payloadHash", "uncompressedBytes", "compression", "payload"] {
            ...
            XCTAssertThrowsError(try HistoryRevisionFormat.decode(JSONSerialization.data(withJSONObject: json)), field)
        }
        var corrupted = data
    ...
    func testRetentionIsIndependentOfLiveStateAndSurvivesReopen() async throws {
        let root = try root(), store = SQLiteStore(directory: root)
    ...
    func testHistoryRequiresLoadedOwnerAndSameDocument() async throws {
        let store = SQLiteStore(directory: try root())
        do { _ = try await store.retain(snapshot(), reason: "Test"); XCTFail("Requires load") } catch { }
        _ = try await store.load()
        do { _ = try await store.retain(snapshot(), reason: "Test"); XCTFail("Requires document identity") } catch {
    ...
    func testProtectingDuplicatePromotesOriginalIdentityWithoutAnotherRevision() async throws {
        ...
        let protected = try await store.retain(value, reason: "Before restore", timestamp: date.addingTimeInterval(100), milestone: true)
    ...
    func testDuplicateMustBeVerifiedBeforeSkippingPreservation() async throws {
        ...
        do { _ = try await store.revision(sequence: first.sequence); XCTFail("Corrupt payload") } catch { }
        try await store.close()
    }
    ...
    func testConcurrentRequestsSerializeAndDeduplicate() async throws {
        ...
        let sequences = try await withThrowingTaskGroup(of: Int64.self) { group in
            for _ in 0..<8 {
                group.addTask { try await store.retain(value, reason: "Idle", timestamp: timestamp).sequence }
            }
            var results: [Int64] = []
    ...
    func testSwappedValidPayloadCannotImpersonateAnotherRow() async throws {
        ...
        let second = try await store.retain(snapshot(documentID: initial.documentID), reason: "Next", timestamp: date)
        try execute(root, "UPDATE history_revisions SET payload=(SELECT payload FROM history_revisions WHERE sequence=\(second.sequence)) WHERE sequence=\(first.sequence)")
        do { _ = try await store.revision(sequence: first.sequence); XCTFail("Swapped envelope") } catch {
            XCTAssertEqual(error as? StoreError, .invalidPayload)
        }
    ...
    func testHistoryWriteFailuresRollbackWithoutAffectingAutosaveOrRecovery() async throws {
        for stage in [StoreStage.historyWrite, .historyVerify, .historyCommit] {
            let root = try root(), seed = SQLiteStore(directory: root)
            ...
            let store = SQLiteStore(directory: root, inject: { if $0 == stage { throw StoreError.injected(stage.rawValue) } })
            _ = try await store.load()
            let latest = snapshot(documentID: initial.documentID)
            _ = try await store.save(latest)
            do { _ = try await store.retain(latest, reason: "Test", timestamp: date); XCTFail("Expected \(stage)") } catch {
    ...
    func testDamagedMetadataOnlyDisablesItsRowAndDoesNotBlockStartup() async throws {
        ...
        try await store.close()
        try execute(root, "UPDATE history_revisions SET metadata=X'00' WHERE sequence=\(first.sequence)")
        let reopened = SQLiteStore(directory: root)
    ...
    func testHistorySchemaDamageDoesNotPreventCurrentStateUse() async throws {
        ...
        do { _ = try await reopened.revisions(); XCTFail("Expected malformed history schema") } catch {
    ...
    func testPagingUsesSequenceNotClockAndSettingsAreValidated() async throws {
        ...
        let initial = try await store.load()
        let first = try await store.retain(initial, reason: "Initial", timestamp: date)
        let second = try await store.retain(snapshot(documentID: initial.documentID), reason: "Next", timestamp: date.addingTimeInterval(-10))
        ...
        do { try await store.setHistorySettings(HistorySettings(byteBudget: -1)); XCTFail("Invalid budget") } catch { }
    ...
    func testVersionThreeMigrationPreservesCompleteCurrentState() async throws {
        ...
        try await seed.close()
        try execute(root, "DROP TABLE history_revisions; DROP TABLE history_settings; PRAGMA user_version=3")
        ...
        XCTAssertEqual(try Data(contentsOf: XCTUnwrap(backups.first).appendingPathComponent("Jort.sqlite")), original)
        try await store.close()
    }
    ...
    func testPruningProtectsRecentMilestonesAndCurrentState() async throws {
        ...
        for index in 0..<6 {
            _ = try await store.retain(snapshot(documentID: current.documentID, text: "Version \(index)"), reason: "Idle")
        }
    ...
    func testPruneFailureRollsBackAllDeletes() async throws {
        ...
        let initial = try await seed.load()
        for index in 0..<4 {
            _ = try await seed.retain(snapshot(documentID: initial.documentID, text: "Version \(index)"), reason: "Idle")
        }
        try await seed.setHistorySettings(HistorySettings(byteBudget: 1, minimumRecentCount: 1))
        let expected = try await seed.revisions()
        try await seed.close()
        let failing = SQLiteStore(directory: root, inject: { if $0 == .historyPrune { throw StoreError.sqlite(13, "disk full") } })
        _ = try await failing.load()
        do { _ = try await failing.pruneHistory(); XCTFail("Expected failure") } catch { }
    ...
    func testRecoveryCarriesVerifiedHistoryAndReportsDamagedEntries() async throws {
        ...
        try await store.setHistorySettings(settings)
        try await store.close()
        try execute(root, "UPDATE current_state SET payload=X'00'; UPDATE history_revisions SET payload=X'00' WHERE sequence=\(first.sequence)")
        let recovery = SQLiteStore(directory: root)
    ...
    @MainActor func testCoordinatorCoalescesTypingAndRetainsSemanticBoundariesInOrder() async throws {
        ...
        for index in 0..<20 {
            try owner.apply(.init(baseRevision: owner.snapshot.revision, origin: .native,
        ...
        try owner.apply(.init(baseRevision: owner.snapshot.revision, origin: .metadata,
        ...
        XCTAssertEqual(entries.compactMap { $0.metadata?.reason }, ["Landmark changed", "Bulk change", "Initial state"])
    ...
    @MainActor func testIdleBoundaryUsesInjectedClockAndDelay() async throws {
        ...
        let initial = try await store.load()
        let timestamp = date
        let coordinator = HistoryCoordinator(store: store, initial: initial, now: { timestamp },
            sleep: { interval in
    ...
    @MainActor func testHistoryFailureDoesNotChangeAutosaveHealth() async throws {
        ...
        try await seed.close()
        let store = SQLiteStore(directory: root, inject: { if $0 == .historyWrite { throw StoreError.sqlite(13, "disk full") } })
    ...
    func testRepeatedPruningBoundsRetainedBytesAndReusesDatabaseSpace() async throws {
        ...
        try await store.setHistorySettings(HistorySettings(byteBudget: 1, minimumRecentCount: 2))
        for index in 0..<30 {
            let value = snapshot(documentID: initial.documentID, text: String(repeating: "Revision \(index) 🦊 ", count: 100))
        ...
        try await store.close()
        let attributes = try FileManager.default.attributesOfItem(atPath: root.appendingPathComponent("Store/Jort.sqlite").path)
    ...
    func testRecoveryDestinationFailurePreservesOriginalHistoryDatabase() async throws {
        ...
        let failing = SQLiteStore(directory: root, inject: { if $0 == .historyWrite { throw StoreError.sqlite(13, "disk full") } })
        do { _ = try await failing.recover(); XCTFail("Expected failure") } catch { }
    ...
    @MainActor func testLifecycleDrainsEditsArrivingDuringHistoryCompletion() async throws {
        ...
        let newest = snapshot(documentID: initial.documentID, text: "Arrived during history", generation: 8)
