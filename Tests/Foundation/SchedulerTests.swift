...
private actor ControlledStore: DocumentStore {
    ...
    func save(_ snapshot: DocumentSnapshot) async throws -> Int64 {
        ...
        saved = snapshot; return snapshot.revision
    }
}
final class SchedulerTests: XCTestCase {
    @MainActor private func load(_ controller: PersistenceController) async throws -> DocumentCoordinator {
        let snapshot = try await withCheckedThrowingContinuation { continuation in controller.load { continuation.resume(with: $0) } }
    ...
    @MainActor func testFailureBudgetAndManualRetryLatestSnapshot() async throws {
        ...
        controller.onState = { if case .saveFailed(_, let failure) = $0, failure.retriesRemaining == 0 { failures.fulfill() } }
        try owner.apply(.init(baseRevision: 0, origin: .native, mutation: .edit(text: "keep me", range: nil, replacementLength: nil)))
        controller.changed(owner.snapshot); controller.flush()
    ...
    @MainActor func testRapidFlushesAndEditsDuringSaveDrainLatest() async throws {
        ...
        try owner.apply(.init(baseRevision: 0, origin: .native, mutation: .edit(text: "first", range: nil, replacementLength: nil)))
        ...
        for _ in 0..<20 { persistence.flush { XCTAssertTrue($0); finished.fulfill() } }
        try owner.apply(.init(baseRevision: 1, origin: .native, mutation: .edit(text: "second", range: nil, replacementLength: nil)))
        persistence.changed(owner.snapshot)
        await fulfillment(of: [finished], timeout: 5)
        XCTAssertEqual(owner.committedRevision, 2)
        let actual = await store.saved; XCTAssertEqual(actual.text, "second")
    }
    @MainActor func testContinuousEditingSavesBeforeIdle() async throws {
        ...
        let owner = try await load(controller)
        for index in 0..<12 {
            try owner.apply(.init(baseRevision: owner.snapshot.revision, origin: .native, mutation: .edit(text: String(repeating: "x", count: index + 1), range: nil, replacementLength: nil)))
            controller.changed(owner.snapshot)
