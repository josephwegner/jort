...
final class ToolPackageTests: XCTestCase {
    ...
    private func write(_ package: ToolPackage, named name: String, to root: URL) throws -> URL {
        ...
        try JSONEncoder().encode(package.manifest).write(to: directory.appendingPathComponent("tool.json"))
        try Data(package.source.utf8).write(to: directory.appendingPathComponent("tool.js"))
        return directory
    }
    private func writeIndex(installed: URL, entries: [String: String], disabled: Set<String> = []) throws {
        try FileManager.default.createDirectory(at: installed, withIntermediateDirectories: true)
        let value: [String: Any] = ["schemaVersion": 1, "installed": entries, "disabled": Array(disabled)]
        try JSONSerialization.data(withJSONObject: value).write(to: installed.appendingPathComponent("index.json"))
    }
    ...
    func testMalformedBundleIsolationAndSymlinkRejection() async throws {
        ...
        XCTAssertNil(invalid.packageID); XCTAssertEqual(invalid.origin, .bundled); XCTAssertTrue(invalid.isEnabled)
    ...
    func testBundledUpgradeAndInvalidOverridePreserveUserFiles() async throws {
        ...
        XCTAssertEqual(invalid.packageID, user.manifest.id); XCTAssertTrue(invalid.isOverride)
        XCTAssertEqual(invalid.validation, .invalid); XCTAssertTrue(invalid.isEnabled)
    ...
    func testInspectionIdentifiesMalformedKnownAndUnknownBundles() async throws {
        let shipped = try root(), installed = try root()
        let known = shipped.appendingPathComponent("known"), unknown = shipped.appendingPathComponent("unknown")
        ...
        let snapshot = try await ToolPackageRegistry(bundledDirectory: shipped, installedDirectory: installed).inspect()
        XCTAssertTrue(snapshot.packages.isEmpty); XCTAssertTrue(snapshot.executable.isEmpty)
        ...
        XCTAssertTrue(snapshot.candidates.allSatisfy { $0.validation == .invalid && $0.isEnabled && $0.diagnostic != nil })
    }

    func testDuplicateBundledIDsExposeConflictingCandidates() async throws {
        let shipped = try root(), installed = try root()
        let package = try ToolPackage.load(from: bundled.appendingPathComponent("calc"))
        _ = try write(package, named: "first", to: shipped); _ = try write(package, named: "second", to: shipped)
        let snapshot = try await ToolPackageRegistry(bundledDirectory: shipped, installedDirectory: installed).inspect()
        XCTAssertTrue(snapshot.packages.isEmpty); XCTAssertTrue(snapshot.executable.isEmpty)
        XCTAssertEqual(snapshot.candidates.map(\.packageID), [package.manifest.id, package.manifest.id])
        XCTAssertTrue(snapshot.candidates.allSatisfy { $0.origin == .bundled && $0.validation == .conflicting && $0.isEnabled })
    ...
    func testCommandConflictIncludesDisabledCandidateAndPreservesExecutableProjection() async throws {
        ...
        var second = first; second.manifest.id = "org.test.second"; second.manifest.name = "Second"
        _ = try write(first, named: "first", to: shipped); _ = try write(second, named: "second", to: shipped)
        let unrelated = ToolPackage(manifest: .init(id: "org.test.unrelated", name: "Unrelated", command: "/other"), source: first.source)
