...
private actor RecordingTransport: OpenRouterTransport {
    ...
    func send(_ request: URLRequest, maximumBytes: Int) throws -> Data {
        ...
        guard data.count <= maximumBytes else { throw ModelFailure.limit }; return data
    }
}

final class ModelToolTests: XCTestCase {
    private func package(mode: ToolInputMode = .contained) -> ToolPackage {
        var manifest = ToolManifest(id: "dev.test.model", name: "Model", command: "/model", inputMode: mode)
        manifest.schemaVersion = 2; manifest.executor = .model; manifest.modelID = ModelCatalog.defaultModelID
    ...
    func testLegacyDecodingAndDiscriminatedImplementation() throws {
        ...
        XCTAssertNoThrow(try ToolRuntime.validate(.init(manifest: decoded, source: "export default async function(input) { return {output: input.content}; }")))
        XCTAssertNoThrow(try ToolRuntime.validate(package()))
        XCTAssertEqual(package().source, "")
        XCTAssertThrowsError(try ToolPackage(manifest: package().manifest, source: "executable").validate())
        var invalid = package(); invalid.manifest.schemaVersion = 1
        XCTAssertThrowsError(try invalid.validate())
        invalid = package(); invalid.implementation = .model(instructions: "\0")
        XCTAssertThrowsError(try invalid.validate())
    }
    func testPackageRoundTripUnknownModelAndConflictingCommands() async throws {
        let directory = try root(), registry = ToolPackageRegistry(bundledDirectory: directory.appendingPathComponent("empty"), installedDirectory: directory)
        var value = package()
        _ = try await registry.save(value)
        let reloaded = try await ToolPackageRegistry(bundledDirectory: directory.appendingPathComponent("empty"), installedDirectory: directory).inspect()
        XCTAssertEqual(reloaded.executable, [value])
        value.manifest.id = "dev.test.conflict"
        do { _ = try await registry.save(value); XCTFail() } catch { XCTAssertEqual(error as? ToolPackageError, .conflict) }
        value = package(); value.manifest.modelID = "removed/model"
    ...
    func testModelSettingsPersistenceAndAncestry() async throws {
        ...
        let preferences = SQLiteSettingsStore(directory: directory), store = PackageSettingsStore(registry: registry, preferences: preferences)
        _ = try await store.load()
        var definition = UserToolDefinition(id: ToolID("dev.test.custom"), basedOnTemplateID: ToolID("dev.jort.ask"), displayName: "Custom", commandName: "custom", source: "", isEnabled: false)
        definition.manifest = package().manifest; definition.instructions = package().instructions
    ...
    func testCatalogBoundsFilteringAndPreservedUnavailableSelection() throws {
        XCTAssertEqual(ModelCatalog.bundled.filter("anthropic").map(\.id), ["anthropic/claude-sonnet-4.6"])
        XCTAssertTrue(ModelCatalog.bundled.filter("does not exist").isEmpty)
        XCTAssertEqual(try ModelCatalog.decode(JSONEncoder().encode(ModelCatalog.bundled)), ModelCatalog.bundled)
        ...
        definition.manifest = package().manifest; definition.instructions = "Do something"
    ...
    func testPreflightDoesNotInitializeProviderAndDispatcherCapturesExactInput() async throws {
        ...
        let none = await fake.requests; XCTAssertTrue(none.isEmpty)
        ...
        XCTAssertEqual(captured.count, 1); XCTAssertEqual(captured.first?.content, "秘密\ntext")
    ...
    func testOpenRouterRequestAndMalformedResponseBounds() async throws {
        let transport = RecordingTransport([.success(data(#"{"choices":[{"finish_reason":"stop","message":{"content":"answer","tool_calls":null,"function_call":null}}],"usage":{"completion_tokens":2}}"#))])
        let provider = OpenRouterProvider(credentials: MemoryModelCredentialStore("secret"), transport: transport)
        let request = try ModelRequest(package: package(), content: "exact")
        let result = try await provider.execute(request); XCTAssertEqual(result, "answer")
        ...
        for malformed in [#"{"choices":[]}"#, #"{"choices":[{"finish_reason":"tool_calls","message":{"content":"","tool_calls":[]}}],"usage":{"completion_tokens":1}}"#, #"{"choices":[{"finish_reason":"length","message":{"content":"partial"}}],"usage":{"completion_tokens":1}}"#] {
            XCTAssertThrowsError(try OpenRouterProvider.decode(data(malformed), request: request))
        }
        var limited = package(); limited.manifest.maximumOutputLines = 1; limited.manifest.maximumOutputBytes = 12
        let bound = try ModelRequest(package: limited, content: "")
        XCTAssertThrowsError(try bound.validateOutput("a\nb")); XCTAssertThrowsError(try bound.validateOutput(String(repeating: "é", count: 7)))
    }
    func testCancellationDropsLateFakeOutput() async throws {
        let fake = FakeModelProvider(.late("late", .seconds(2)))
        let dispatcher = ToolExecutorDispatcher(modelAvailable: { true }, provider: { fake }), value = package()
        ...
        let result = await task.value; XCTAssertNil(result.output); XCTAssertEqual(result.error, ModelFailure.cancelled.message)
    }
    func testPKCECorrelationExpirationAndEntropy() throws {
        let attempt = try PKCEAttempt(), other = try PKCEAttempt()
        XCTAssertEqual(attempt.verifier.count, 43); XCTAssertEqual(attempt.challenge.count, 43)
        XCTAssertNotEqual(attempt.verifier, other.verifier); XCTAssertNotEqual(attempt.nonce, other.nonce)
        XCTAssertEqual(try attempt.code(from: "/callback/\(attempt.nonce)?code=test"), "test")
        for target in ["/callback/wrong?code=test", "/callback/\(attempt.nonce)?code=a&code=b", "/callback/\(attempt.nonce)?code="] {
            XCTAssertThrowsError(try attempt.code(from: target))
        }
        XCTAssertThrowsError(try attempt.code(from: "/callback/\(attempt.nonce)?code=test", now: attempt.expiresAt))
    ...
    func testVerifiedReplacementRollbackAndSecretIsolation() async throws {
        let directory = try root(), settings = SQLiteSettingsStore(directory: directory)
        _ = try await settings.load()
        let credentials = MemoryModelCredentialStore("old-secret")
        let transport = RecordingTransport([.success(data(#"{"key":"new-secret"}"#)), .success(data(#"{"data":{"label":"new-secret"}}"#)), .success(data(#"{"key":"bad-secret"}"#)), .failure(.authentication)])
        let connection = OpenRouterConnection(credentials: credentials, settings: settings, transport: transport)
        try await connection.exchange(code: "auth-code", attempt: PKCEAttempt())
        let first = await credentials.read(); XCTAssertEqual(first, "new-secret")
        do { try await connection.exchange(code: "auth-code-2", attempt: PKCEAttempt()); XCTFail() } catch {}
        let retained = await credentials.read(); XCTAssertEqual(retained, "new-secret")
        let snapshot = await settings.currentSnapshot()
        let status = snapshot.preferences["openrouter.connection"] ?? ""
        for secret in ["old-secret", "new-secret", "bad-secret", "auth-code"] { XCTAssertFalse(status.contains(secret)) }
        try await connection.disconnect()
        let removed = await credentials.read(); XCTAssertNil(removed)
    ...
    func testKeychainFaultAndOfflineCheckPreserveCredential() async throws {
        ...
        let transport = RecordingTransport([.success(data(#"{"key":"replacement"}"#)), .success(data(#"{"data":{"label":"label"}}"#)), .failure(.offline)])
        let connection = OpenRouterConnection(credentials: credentials, transport: transport)
        do { try await connection.exchange(code: "code", attempt: PKCEAttempt()); XCTFail() } catch { XCTAssertEqual(error as? ModelFailure, .credentialStore) }
        let retained = await credentials.read(); XCTAssertEqual(retained, "original")
        do { try await connection.check(); XCTFail() } catch {}
        let status = await connection.currentStatus(); XCTAssertEqual(status.state, .unableToVerify)
    }
    func testBundledTemplatesContractsAndDuplication() throws {
        let root = URL(fileURLWithPath: #filePath).deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent().appendingPathComponent("Jort/Resources/Tools")
        for (name, mode, operation) in [("ask", ToolInputMode.ephemeralMultiline, ToolOutputOperation.insertAtInvocation), ("rewrite", .contextual, .replaceContext)] {
            let package = try ToolPackage.load(from: root.appendingPathComponent(name))
            XCTAssertEqual(package.manifest.inputMode, mode); XCTAssertEqual(package.manifest.outputOperation, operation)
            XCTAssertEqual(package.manifest.executorType, .model)
            var template = ToolTemplate(id: ToolID(package.manifest.id), displayName: name, commandName: name, source: "")
            template.manifest = package.manifest; template.instructions = package.instructions
...
extension ModelToolTests {
    func testOAuthReplayCancellationAndMalformedExchange() async throws {
        let credentials = MemoryModelCredentialStore("original")
        let transport = RecordingTransport([.success(data(#"{"key":"new"}"#)), .success(data(#"{"data":{"label":"label"}}"#)), .success(data(#"{"key":null}"#))])
        let connection = OpenRouterConnection(credentials: credentials, transport: transport), attempt = try PKCEAttempt()
        try await connection.exchange(code: "code", attempt: attempt)
        do { try await connection.exchange(code: "code", attempt: attempt); XCTFail("replayed attempt") } catch { XCTAssertEqual(error as? ModelFailure, .authorization) }
        do { try await connection.exchange(code: "code", attempt: PKCEAttempt()); XCTFail("malformed key") } catch { XCTAssertEqual(error as? ModelFailure, .malformed) }
        let retained = await credentials.read(); XCTAssertEqual(retained, "new")
        let task = Task { try await connection.connect(openBrowser: { _ in false }) }
        do { try await task.value; XCTFail() } catch {}
        let unchanged = await credentials.read(); XCTAssertEqual(unchanged, "new")
    }
    func testLoopbackCallbackSuccessAndOneShotConsumption() async throws {
        ...
        let url = URL(string: "http://127.0.0.1:\(callback.port)/callback/\(attempt.nonce)?code=bounded-code")!
        let (_, response) = try await URLSession.shared.data(from: url)
        XCTAssertEqual((response as? HTTPURLResponse)?.statusCode, 200)
        let code = try await receiver.value; XCTAssertEqual(code, "bounded-code")
        do { _ = try await callback.receive(attempt); XCTFail() } catch { XCTAssertEqual(error as? ModelFailure, .authorization) }
    }
    func testLoopbackCancellationAndExpiredAttempt() async throws {
        let callback = try LoopbackOAuthCallback(), attempt = try PKCEAttempt()
        let receiver = Task { try await callback.receive(attempt) }
        try await Task.sleep(for: .milliseconds(20)); receiver.cancel()
        do { _ = try await receiver.value; XCTFail() } catch {}
        let expired = try LoopbackOAuthCallback()
        do { _ = try await expired.receive(PKCEAttempt(now: Date().addingTimeInterval(-200))); XCTFail() }
    ...
    func testModelFilteringValidationAndDispatchPerformance() async throws {
        ...
        for _ in 0..<100 {
            var start = CFAbsoluteTimeGetCurrent(); _ = ModelCatalog.bundled.filter("GPT")
            filtering.append((CFAbsoluteTimeGetCurrent() - start) * 1000)
            start = CFAbsoluteTimeGetCurrent(); _ = try ModelRequest(package: value, content: "input")
            validation.append((CFAbsoluteTimeGetCurrent() - start) * 1000)
            start = CFAbsoluteTimeGetCurrent(); let result = await dispatcher.execute(value, input: .init(content: "input"))
            dispatch.append((CFAbsoluteTimeGetCurrent() - start) * 1000); XCTAssertEqual(result.output, "answer")
        }
        func p95(_ values: [Double]) -> Double { values.sorted()[94] }
        print("PERF Model ms p95: filtering=\(p95(filtering)), validation=\(p95(validation)), dispatch=\(p95(dispatch))")
        XCTAssertLessThan(p95(filtering), 100); XCTAssertLessThan(p95(validation), 100); XCTAssertLessThan(p95(dispatch), 100)
    }
}

extension ModelToolTests {
    func testCatalogProjectionRetainsModelImplementationAndConflicts() throws {
        var definition = UserToolDefinition(id: ToolID("dev.test.model"), displayName: "Model", commandName: "model", source: "", isEnabled: true)
        definition.manifest = package().manifest; definition.instructions = package().instructions
        ...
        var other = definition; other.id = ToolID("dev.test.other")
        XCTAssertTrue(builder.executableTools(templates: [], snapshot: .init(customTools: [definition, other])).isEmpty)
    }
    func testUnsupportedOriginIsRejectedBeforeCredentialTransmission() async throws {
        for address in ["http://openrouter.ai/api/v1/key", "https://example.com/api/v1/key", "https://openrouter.ai:8443/api/v1/key", "https://openrouter.ai/not-an-endpoint"] {
            do { _ = try await BoundedOpenRouterTransport().send(URLRequest(url: URL(string: address)!), maximumBytes: 1024); XCTFail() }
...
extension ModelToolTests {
    func testSQLiteModelDefinitionRoundTrip() async throws {
        let directory = try root(), store = SQLiteSettingsStore(directory: directory)
        _ = try await store.load()
        var definition = UserToolDefinition(id: ToolID("dev.test.model"), displayName: "Model", commandName: "model", source: "", isEnabled: true)
        definition.manifest = package().manifest; definition.instructions = package().instructions
        _ = try await store.save(definition, expectedRevision: nil)
        try await store.close()
        let reopened = SQLiteSettingsStore(directory: directory), snapshot = try await reopened.load()
...
extension ModelToolTests {
    func testSQLiteVersionOneMigrationPreservesLegacySourceIdentityAndAncestry() async throws {
        ...
        XCTAssertEqual(sqlite3_open(location.appendingPathComponent("Settings.sqlite").path, &db), SQLITE_OK)
        let sql = """
        ...
        let store = SQLiteSettingsStore(directory: directory), snapshot = try await store.load()
        let definition = try XCTUnwrap(snapshot.customTools.first)
        XCTAssertEqual(definition.id, ToolID("dev.test.legacy")); XCTAssertEqual(definition.revision, RecordRevision(3))
        XCTAssertEqual(definition.basedOnTemplateID, ToolID("dev.jort.calc")); XCTAssertTrue(definition.isEnabled)
        XCTAssertEqual(definition.source, "export default async function(input) { return {output: input.content}; }")
        XCTAssertNil(definition.manifest); XCTAssertNil(definition.instructions)
