...
@MainActor final class DocumentSearchWorkspaceTests: XCTestCase {
    ...
    private func editor() async throws -> (EditorViewController, NSWindow) {
        ...
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("SearchWorkspace-\(UUID())")
        let editor = EditorViewController(persistence: PersistenceController(store: SQLiteStore(directory: root)))
        ...
        window.contentViewController = editor; window.setContentSize(NSSize(width: 920, height: 680)); window.makeKeyAndOrderFront(nil)
    ...
    func testLazyPaletteSearchKeyboardNavigationAndWrappedMatch() async throws {
        ...
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: text, range: nil, replacementLength: nil)))
        ...
        search.query.stringValue = "needle"; search.refreshQuery()
        ...
        XCTAssertTrue(search.model.results.allSatisfy { !$0.snippet.contains("↵") && !$0.snippet.contains("\n") })
        ...
        try XCTUnwrap(image.representation(using: .png, properties: [:])).write(to: URL(fileURLWithPath: "/private/tmp/jort-document-search.png"))
        XCTAssertTrue(search.control(search.query, textView: NSTextView(), doCommandBy: #selector(NSResponder.moveDown(_:))))
        let match = search.model.results[1]
        XCTAssertTrue(search.control(search.query, textView: NSTextView(), doCommandBy: #selector(NSResponder.insertNewline(_:))))
    ...
    func testStaleSelectionRefreshesWithoutNavigating() async throws {
        let (editor, window) = try await editor(); defer { window.orderOut(nil) }
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: "needle", range: nil, replacementLength: nil)))
        editor.showDocumentSearch(); let search = try XCTUnwrap(editor.documentSearch)
        search.query.stringValue = "needle"; search.refreshQuery()
        try await waitUntil { search.model.results.count == 1 }
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: "changed", range: nil, replacementLength: nil)))
    ...
    func testSupersededQueriesOptionsAndCancellation() async throws {
        ...
        try owner.apply(.init(baseRevision: 0, origin: .native, mutation: .edit(text: "cat Cat scatter dog", range: nil, replacementLength: nil)))
        let model = DocumentSearchModel()
        model.search(snapshot: owner.snapshot, query: "dog", options: .init())
        model.search(snapshot: owner.snapshot, query: "cat", options: .init(caseSensitive: true, wholeWord: true))
    ...
    func testDismissalAndIMECompositionDoNotExecuteSearchResult() async throws {
        ...
        composing.setMarkedText("に", selectedRange: NSRange(location: 1, length: 0), replacementRange: NSRange(location: NSNotFound, length: 0))
        XCTAssertFalse(search.control(search.query, textView: composing, doCommandBy: #selector(NSResponder.insertNewline(_:))))
        XCTAssertNotNil(editor.documentSearch)
        search.dismiss()
        XCTAssertNil(editor.documentSearch)
        XCTAssertTrue(window.firstResponder === editor.textView)
