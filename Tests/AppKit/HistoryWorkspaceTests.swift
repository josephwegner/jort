...
@MainActor final class HistoryWorkspaceTests: XCTestCase {
    func testWorkspaceConstruction() {
        _ = NSApplication.shared
        let store = SQLiteStore(directory: FileManager.default.temporaryDirectory.appendingPathComponent("HistoryConstruction-\(UUID())"))
        let workspace = HistoryWorkspaceController(store: store)
        _ = workspace.view
        workspace.model.cancel()
    }
    private func editor(inject: @escaping @Sendable (StoreStage) throws -> Void = { _ in }) async throws -> (EditorViewController, NSWindow, any HistoryStore) {
        _ = NSApplication.shared
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("HistoryWorkspace-\(UUID())")
        let controller = EditorViewController(persistence: PersistenceController(store: SQLiteStore(directory: root, inject: inject)))
        let window = NSWindow(contentRect: NSRect(x: 0, y: 0, width: 920, height: 680), styleMask: [.titled], backing: .buffered, defer: false)
        window.isReleasedWhenClosed = false; window.appearance = NSAppearance(named: .darkAqua)
        window.contentViewController = controller; window.setContentSize(NSSize(width: 920, height: 680)); window.makeKeyAndOrderFront(nil)
        for _ in 0..<500 {
    ...
    func testPreviewModesFooterDismissalAndResize() async throws {
        let (editor, window, store) = try await editor(); defer { window.orderOut(nil) }
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: "First thought\nSecond thought\nThird thought", range: nil, replacementLength: nil)))
        editor.textView.history.beginUndoGrouping()
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .metadata, mutation: .landmark(Landmark(lineID: editor.state.lines[1].id, emoji: "🌲"))))
        editor.textView.history.endUndoGrouping()
        let entry = try await store.retain(editor.state, reason: "Landmark changed", timestamp: Date(), milestone: false)
        ...
        XCTAssertEqual((workspace.snapshotScroll.verticalRulerView as? LineRuler)?.landmarks, before.landmarks)
        workspace.mode.selectedSegment = 1; workspace.selectPresentation()
        XCTAssertFalse(workspace.snapshotScroll.isHidden)
        workspace.mode.selectedSegment = 0; workspace.selectPresentation()
        ...
        let changedCell = try XCTUnwrap(workspace.tableView(workspace.changes, viewFor: nil, row: 1) as? HistoryDiffCell)
        ...
        for size in [NSSize(width: 460, height: 300), NSSize(width: 920, height: 680)] {
            window.setContentSize(size); editor.view.layoutSubtreeIfNeeded()
        ...
        let image = try XCTUnwrap(workspace.view.bitmapImageRepForCachingDisplay(in: workspace.view.bounds))
        workspace.view.cacheDisplay(in: workspace.view.bounds, to: image)
        try XCTUnwrap(image.representation(using: .png, properties: [:])).write(to: URL(fileURLWithPath: "/private/tmp/jort-history-workspace.png"))
    ...
    func testRestorePreservesCurrentStateAndUndoRedoAsOneOperation() async throws {
        ...
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: "Do not lose this", range: nil, replacementLength: nil)))
        editor.textView.history.beginUndoGrouping()
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .metadata, mutation: .landmark(Landmark(lineID: editor.state.lines[0].id, emoji: "🦊"))))
        editor.textView.history.endUndoGrouping()
        let before = editor.state
        editor.textView.setSelectedRange(NSRange(location: 2, length: 3))
        editor.showHistory(); try await waitUntil { editor.historyWorkspace != nil }
        ...
        for entry in retained {
            ...
            if revision.snapshot.text == before.text && revision.snapshot.landmarks == before.landmarks { preserved = true }
        }
        XCTAssertTrue(preserved)
        let saved = await withCheckedContinuation { continuation in
            editor.persistence.flushLifecycle(reason: .windowClosed) { continuation.resume(returning: $0) }
        }
        XCTAssertTrue(saved)
    ...
    func testNoBaselineAndSupersededSelection() async throws {
        ...
        model.loadMore(); try await waitUntil { model.selected != nil && !model.message.contains("Looking") }
        XCTAssertNil(model.comparison)
        let first = try XCTUnwrap(model.selectedSequence)
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: "Newer", range: nil, replacementLength: nil)))
        let newest = try await store.retain(editor.state, reason: "Newer", timestamp: Date(), milestone: false)
        model.select(first); model.select(newest.sequence); model.select(first)
        try await waitUntil { model.selected != nil && !model.loading && !model.message.contains("Looking") }
    ...
    func testFailedPreservationRefusesRestore() async throws {
        ...
        let (editor, window, store) = try await editor { stage in
            if stage == .historyCommit, failing.withLock({ $0 }) { throw StoreError.sqlite(13, "disk full") }
        }
        defer { window.orderOut(nil) }
        let entries = try await store.revisions(before: nil, limit: 1)
        let sequence = try XCTUnwrap(entries.first).sequence
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: "Must survive", range: nil, replacementLength: nil)))
        let before = editor.state
        editor.showHistory(); try await waitUntil { editor.historyWorkspace != nil }
        failing.withLock { $0 = true }
        do { try await editor.restoreHistory(sequence: sequence); XCTFail("Must refuse restore") } catch { }
        XCTAssertEqual(editor.state, before)
    ...
    func testLiveMutationDuringPreservationRefusesStaleRestore() async throws {
        ...
        let entries = try await store.revisions(before: nil, limit: 1)
        let sequence = try XCTUnwrap(entries.first).sequence
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: "Before", range: nil, replacementLength: nil)))
        editor.showHistory(); try await waitUntil { editor.historyWorkspace != nil }
        var changed = false
        editor.persistence.onHistoryState = {
            guard !changed else { return }; changed = true
            _ = try? editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: "Arrived during restore", range: nil, replacementLength: nil)))
        }
        do { try await editor.restoreHistory(sequence: sequence); XCTFail("Must refuse stale restore") } catch { }
        XCTAssertTrue(changed)
        XCTAssertEqual(editor.state.text, "Arrived during restore")
        editor.persistence.onHistoryState = nil; editor.dismissHistory()
    }
    ...
    func testCorruptRevisionDisablesPreviewAndRestore() async throws {
        ...
        XCTAssertEqual(sqlite3_open(directory.appendingPathComponent("Store/Jort.sqlite").path, &db), SQLITE_OK)
        XCTAssertEqual(sqlite3_exec(db, "UPDATE history_revisions SET payload=X'00'", nil, nil, nil), SQLITE_OK)
        sqlite3_close(db)
        let before = editor.state
        editor.showHistory(); try await waitUntil { editor.historyWorkspace != nil }
        let workspace = try XCTUnwrap(editor.historyWorkspace)
    ...
    func testConfirmationCancelAndKeyboardExpansion() async throws {
        ...
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: originalText, range: nil, replacementLength: nil)))
        _ = try await store.retain(editor.state, reason: "Earlier", timestamp: Date(), milestone: false)
        try editor.apply(.init(baseRevision: editor.state.revision, origin: .native, mutation: .edit(text: originalText + "\nNew line", range: nil, replacementLength: nil)))
        _ = try await store.retain(editor.state, reason: "Latest", timestamp: Date(), milestone: false)
        let before = editor.state
        editor.showHistory(); try await waitUntil { editor.historyWorkspace?.model.comparison != nil }
        let workspace = try XCTUnwrap(editor.historyWorkspace)
        ...
        let collapsed = (0..<count).first { index in
            ...
            return cell?.subviews.contains { ($0 as? NSButton)?.title.contains("unchanged lines") == true } == true
        }
    ...
    func testTypingThenLandmarkPreservesAndRetainsBoth() async throws {
        ...
        let saved = await withCheckedContinuation { continuation in
            editor.persistence.flushLifecycle(reason: .deactivation) { continuation.resume(returning: $0) }
        }
        XCTAssertTrue(saved)
        let entries = try await store.revisions(before: nil, limit: 1)
    ...
    func testNewLinesKeepEveryVisibleOrdinalWithoutScrollingOrLandmarks() async throws {
        let (editor, window, _) = try await editor(); defer { window.orderOut(nil) }
        for index in 0..<18 {
            editor.textView.history.beginUndoGrouping()
            editor.textView.insertText("Line \(index)\n", replacementRange: NSRange(location: editor.textView.string.utf16.count, length: 0))
            editor.textView.history.endUndoGrouping()
            try await Task.sleep(for: .milliseconds(10))
    ...
    func testHistoryLoadsAnotherPageOnScrollAndVisibleRowCounts() async throws {
        ...
        for index in 0..<104 {
            try owner.apply(.init(baseRevision: owner.snapshot.revision, origin: .native, mutation: .edit(text: "Revision \(index)", range: nil, replacementLength: nil)))
            _ = try await store.retain(owner.snapshot, reason: "Typing paused", timestamp: Date(), milestone: false)
        }
        editor.showHistory(); try await waitUntil { editor.historyWorkspace?.model.entries.count == 100 }
        let workspace = try XCTUnwrap(editor.historyWorkspace)
        try await waitUntil { workspace.model.summaries[workspace.model.entries[0].sequence] != nil }
        if case .changes(let added, let removed) = workspace.model.summaries[workspace.model.entries[0].sequence] {
            XCTAssertEqual(added, 1); XCTAssertEqual(removed, 1)
        } else { XCTFail("Expected inline change counts") }
