...
- A new Settings panel, marketplace/distribution UI, or permissions for network, filesystem, process, shell, native bridging, plugins, or external executables.
...
The manifest contains a stable reverse-DNS-style tool ID, positive integer package version, display name, slash command, description, script-entry contract version, one input mode, one output operation, and byte and line output caps. The input mode is the complete input-shape contract; there is no separate input-shape field. Input modes are `contained`, `contextual`, `ephemeralSingleLine`, and `ephemeralMultiline`. Output operations initially include `replace-invocation`, `replace-context`, and `insert-at-invocation`; defaults are `replace-invocation` for contained input, `replace-context` for contextual input, and `insert-at-invocation` for ephemeral input. Merge confirmation is always required.

The manifest, rather than arbitrary script behavior, determines invocation presentation and merge semantics. Each package exposes one asynchronous main script entry point and may expose one asynchronous `validate` entry point. Both receive the same deeply frozen captured input object containing exactly one `content` string, explicit captured deterministic host facilities, and cancellation state. Validation runs before source is locked; a validation failure is presentation-only and creates no document mutation or history boundary. The main entry returns a bounded structured result containing output text or a structured failure and cannot mutate the document directly. Commands have no separately parsed arguments. After extracting content, the editor removes exactly one leading U+0020 space before validation and execution, preserving every other scalar and leaving canonical text unchanged.
...
The lazy command registry exposes validated package identities and manifest presentation contracts without eagerly evaluating `tool.js` implementations. The parser observes committed text and offers only commands from enabled valid packages; invocation recognition occurs when the user explicitly accepts completion. Pending state stores stable line identities and line-relative UTF-16 anchors rather than relying on untracked global offsets. Unmatched, unaccepted, abandoned, copied, or pasted slash text remains ordinary text with no inferred rich semantics.

### 4. Make completion acceptance distinct from execution

Typing `/` opens filtered command completion. Space or Return accepts a selected command without running it. Space and Return both insert a canonical space after the command; Return does not insert a newline during acceptance. For ephemeral modes the separator is outside the owned token range, and focus transfers into the popover. Once completion closes, Return resumes its native meaning and inserts a newline in multiline canonical input. Shift-Return and the visible Run action dispatch one identical execution path only while invocation-owned content or a contextual boundary is focused; Shift-Return does nothing during unrelated document editing.
...
**Contextual canonical input.** Accepting the command places scope boundaries at the beginning and end of its current logical line, excluding the newline. The user can move either boundary to any legal character position by dragging text-selection-style handles or by using Command-Option with Left, Right, Up, or Down (Option-Shift remains an alias). Before a boundary has focus, Left/Up selects the start and Right/Down selects the end; subsequent movement controls that boundary without mouse priming. Ordinary Shift-arrow remains native text selection. Scope is one contiguous canonical range containing exactly one excluded invocation-token range; the executor receives the exact text before and after `/command` concatenated in document order. Jort excludes the command characters when extracting context, then applies the shared single-leading-space normalization; all remaining whitespace and newlines are preserved. Boundaries stop at the nearest legal position before another active invocation and ranges may never overlap.
...
At viewport edges, clamp the prompt horizontally and flip it above its token when the complete form will not fit below. Pending result tint uses a shared, consistently composited color regardless of contextual source overlap. Preserve normal canonical character advance at the source/result seam; reserve space only for noncanonical actions. Pending decoration must include newly appended visible paragraphs at EOF. Undo/Redo restores styling synchronously with canonical text, without a second text replacement that briefly exposes unstyled output. Settings must render its Tools category and give additional window width to the JavaScript editor, not the tool list.

Source and output silhouettes must stop before the next unowned shaped glyph, including midline contextual boundaries; padded selection rectangles are not glyph boundaries. Ephemeral prompts occupy the editor root overlay above canonical glyphs and inline actions, and retain that stacking order when actions are rebuilt.
...
Pending output uses a purple tint and border and joins the green source region as one compound silhouette. Inline output shares a connected seam with its invocation, with noncanonical padding separating source and result glyphs. Output on later fragments or lines connects through a shared edge or short neck aligned to the invocation, rather than appearing as a detached card. Source and output remain semantically distinct ranges inside the connected shape. Multiple invocations on one line retain separate silhouettes and controls.
...
Immediate Undo after output publication behaves like Dismiss and restores inputting state. Undo after Merge restores the prior pending source-plus-output compound state. Unrelated edits participate in ordinary chronological Undo order, so reversing later edits occurs before reversing publication or Merge. Deleting text that intersects pending calls warns “N mergeable responses will be deleted”. Confirmation deletes exactly the selected characters and drops each affected invocation's entire annotation, making surviving text ordinary; Cancel changes nothing. Mixed selections touching processing or error locks remain blocked. This is one undoable transaction. Escape within pending source or output performs Dismiss and returns to editable input.
...
The date, time, and UUID scripts append the exact content string after their generated value. The existing Tools settings panel saves new enabled packages without a leave-editor warning; the Enabled checkbox is visible beside the Tools heading, draft enablement is saved with the draft, and registry updates refresh open editors. The source editor and ruler clip their drawing to their own bounds.

Completion is a compact rounded overlay above TextKit glyph layers, with command names on the left and muted display names on the right. Row highlighting indicates selection. Run displays ⇧↵ close to the wrapper's right edge for canonical inputs and only inside the popover for ephemeral inputs. All action buttons show a pointing-hand cursor and subtle hover tint. Wrappers share visual-line heights and include a contained input's trailing empty line. Replacing canonical text storage always invalidates decoration styling, including the second display pass during Undo. Context handles use directional flags and independent hit areas. Control reservations and source/output padding are presentation attributes only. Readable validation/error explanations occupy noncanonical accessory rows anchored to the invocation line. History rows carry small command/state annotations, and metadata-only changes remain visible in comparisons.
